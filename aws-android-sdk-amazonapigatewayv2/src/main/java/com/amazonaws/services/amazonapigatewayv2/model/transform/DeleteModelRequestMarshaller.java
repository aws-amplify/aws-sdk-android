...
/**
 * JSON request marshaller for DeleteModelRequest
 */
    ...
    public Request<DeleteModelRequest> marshall(DeleteModelRequest deleteModelRequest) {
        if (deleteModelRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteModelRequest)");
        }

        Request<DeleteModelRequest> request = new DefaultRequest<DeleteModelRequest>(
                deleteModelRequest, "AmazonApiGatewayV2");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/v2/apis/{apiId}/models/{modelId}";
        uriResourcePath = uriResourcePath.replace(
                "{apiId}",
                (deleteModelRequest.getApiId() == null) ? "" : StringUtils
                        .fromString(deleteModelRequest.getApiId()));
        uriResourcePath = uriResourcePath.replace(
                "{modelId}",
                (deleteModelRequest.getModelId() == null) ? "" : StringUtils
                        .fromString(deleteModelRequest.getModelId()));
