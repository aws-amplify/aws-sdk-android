...
/**
 * JSON request marshaller for DeleteAuthorizerRequest
 */
    ...
    public Request<DeleteAuthorizerRequest> marshall(DeleteAuthorizerRequest deleteAuthorizerRequest) {
        if (deleteAuthorizerRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteAuthorizerRequest)");
        }

        Request<DeleteAuthorizerRequest> request = new DefaultRequest<DeleteAuthorizerRequest>(
                deleteAuthorizerRequest, "AmazonApiGatewayV2");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/v2/apis/{apiId}/authorizers/{authorizerId}";
        uriResourcePath = uriResourcePath.replace(
                "{apiId}",
                (deleteAuthorizerRequest.getApiId() == null) ? "" : StringUtils
                        .fromString(deleteAuthorizerRequest.getApiId()));
        uriResourcePath = uriResourcePath.replace(
                "{authorizerId}",
                (deleteAuthorizerRequest.getAuthorizerId() == null) ? "" : StringUtils
                        .fromString(deleteAuthorizerRequest.getAuthorizerId()));
