...
/**
 * JSON request marshaller for DeleteDeploymentRequest
 */
    ...
    public Request<DeleteDeploymentRequest> marshall(DeleteDeploymentRequest deleteDeploymentRequest) {
        if (deleteDeploymentRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteDeploymentRequest)");
        }

        Request<DeleteDeploymentRequest> request = new DefaultRequest<DeleteDeploymentRequest>(
                deleteDeploymentRequest, "AmazonApiGatewayV2");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/v2/apis/{apiId}/deployments/{deploymentId}";
        uriResourcePath = uriResourcePath.replace(
                "{apiId}",
                (deleteDeploymentRequest.getApiId() == null) ? "" : StringUtils
                        .fromString(deleteDeploymentRequest.getApiId()));
        uriResourcePath = uriResourcePath.replace(
                "{deploymentId}",
                (deleteDeploymentRequest.getDeploymentId() == null) ? "" : StringUtils
                        .fromString(deleteDeploymentRequest.getDeploymentId()));
