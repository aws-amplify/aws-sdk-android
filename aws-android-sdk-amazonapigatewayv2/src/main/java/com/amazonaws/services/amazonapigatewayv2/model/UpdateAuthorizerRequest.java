...
public class UpdateAuthorizerRequest extends AmazonWebServiceRequest implements Serializable {
     ...
     *         chained together.
     */
    public UpdateAuthorizerRequest withAuthorizerResultTtlInSeconds(
     ...
     *            REQUEST authorizers, this must be a well-formed Lambda
     *            function URI, for example,
     *            arn:aws:apigateway:us-west-2:lambda:
     *            path/2015-03-31/functions/arn
     *            :aws:lambda:us-west-2:<replaceable
     *            >{account_id}</replaceable>:function
     *            :<replaceable>{lambda_function_name
     *            }</replaceable>/invocations. In general, the URI has this
     *            form: arn:aws:apigateway:<replaceable>{region}</replaceable>:
     *            lambda:path/<replaceable>{service_api}</replaceable> , where
     *            <replaceable></replaceable>{region} is the same as the region
     *            hosting the Lambda function, path indicates that the remaining
     *            substring in the URI should be treated as the path to the
     ...
     *         chained together.
     */
    public UpdateAuthorizerRequest withIdentityValidationExpression(
