...
public class GetAuthorizersRequestMarshaller implements
    ...
    public Request<GetAuthorizersRequest> marshall(GetAuthorizersRequest getAuthorizersRequest) {
        ...
        Request<GetAuthorizersRequest> request = new DefaultRequest<GetAuthorizersRequest>(
                getAuthorizersRequest, "AmazonApiGatewayV2");
        request.setHttpMethod(HttpMethodName.GET);

        String uriResourcePath = "/v2/apis/{apiId}/authorizers";
        uriResourcePath = uriResourcePath.replace(
                "{apiId}",
                (getAuthorizersRequest.getApiId() == null) ? "" : StringUtils
                        .fromString(getAuthorizersRequest.getApiId()));
        if (getAuthorizersRequest.getMaxResults() != null) {
            request.addParameter("maxResults",
                    StringUtils.fromString(getAuthorizersRequest.getMaxResults()));
        }
        if (getAuthorizersRequest.getNextToken() != null) {
            request.addParameter("nextToken",
                    StringUtils.fromString(getAuthorizersRequest.getNextToken()));
        }
