...
public class UpdateItemRequest extends AmazonWebServiceRequest implements Serializable {
     ...
     * New applications should use <i>UpdateExpression</i> instead. Do not
     ...
     * exception. <p>This parameter can be used for modifying top-level
     * attributes; however, it does not support individual list or map
     * elements. </important> <p>The names of attributes to be modified, the
    ...
    /**
     * <important> <p>This is a legacy parameter, for backward compatibility.
     * New applications should use <i> ConditionExpression </i> instead. Do
     ...
     * <i>Expected</i> provides a conditional block for the <i>UpdateItem</i>
     ...
     * returned in the response. If set to <code>NONE</code> (the default),
     ...
     * <code>SET myNum = myNum + :val</code> <p><code>SET</code> supports the
     * following functions: <ul> <li><p><code>if_not_exists (path,
     * operand)</code> - if the item does not contain an attribute at the
     * specified path, then <code>if_not_exists</code> evaluates to operand;
     * otherwise, it evaluates to path. You can use this function to avoid
     * overwriting an attribute that may already be present in the item.</li>
     * <li><p><code>list_append (operand, operand)</code> - evaluates to a
     ...
     * operands.</li> </ul> <p>These function names are case-sensitive. </li>
     * <li> <p><code>REMOVE</code> - Removes one or more attributes from an
     * item. </li> <li> <p><code>ADD</code> - Adds the specified value to the
     ...
     * and if <i>Value</i> is also a number, then <i>Value</i> is
     * mathematically added to the existing attribute. If <i>Value</i> is a
     * negative number, then it is subtracted from the existing attribute.
     * <note> <p>If you use <code>ADD</code> to increment or decrement a
     * number value for an item that doesn't exist before the update,
     * DynamoDB uses <code>0</code> as the initial value. <p>Similarly, if
     * you use <code>ADD</code> for an existing item to increment or
     ...
     * <important><p>The <code>DELETE</code> action only supports set data
     * types. In addition, <code>DELETE</code> can only be used on top-level
     * attributes, not nested attributes. </important> </li> </ul> <p>You can
     ...
     * Guide</i>. <note><p><i>UpdateExpression</i> replaces the legacy
     * <i>AttributeUpdates</i> parameter.</note>
     */
     ...
     * attribute_type | contains | begins_with | size</code> <p>These
     * function names are case-sensitive. </li> <li> <p>Comparison operators:
     * <code> = | <> | < | > | <= | >= | BETWEEN | IN</code> </li> <li> <p>
     * Logical operators: <code>AND | OR | NOT</code> </li> </ul> <p>For more
     ...
     * of the schema in the table's attribute definition.
     *
     * @param attributeUpdates <important> <p>This is a legacy parameter, for backward compatibility.
     *         New applications should use <i>UpdateExpression</i> instead. Do not
     *         combine legacy parameters and expression parameters in a single API
     *         call; otherwise, DynamoDB will return a <i>ValidationException</i>
     *         exception. <p>This parameter can be used for modifying top-level
