...
/**
 * JSON marshaller for POJO ClusterTimeline
 */
class ClusterTimelineJsonMarshaller {

    public void marshall(ClusterTimeline clusterTimeline, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (clusterTimeline.getCreationDateTime() != null) {
            java.util.Date creationDateTime = clusterTimeline.getCreationDateTime();
            jsonWriter.name("CreationDateTime");
            jsonWriter.value(creationDateTime);
        }
        if (clusterTimeline.getReadyDateTime() != null) {
            java.util.Date readyDateTime = clusterTimeline.getReadyDateTime();
            jsonWriter.name("ReadyDateTime");
            jsonWriter.value(readyDateTime);
        }
        if (clusterTimeline.getEndDateTime() != null) {
            java.util.Date endDateTime = clusterTimeline.getEndDateTime();
