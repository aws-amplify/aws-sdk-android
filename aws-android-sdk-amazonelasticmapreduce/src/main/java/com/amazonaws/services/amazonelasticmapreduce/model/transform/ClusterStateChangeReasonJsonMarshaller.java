...
/**
 * JSON marshaller for POJO ClusterStateChangeReason
 */
class ClusterStateChangeReasonJsonMarshaller {

    public void marshall(ClusterStateChangeReason clusterStateChangeReason, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (clusterStateChangeReason.getCode() != null) {
            String code = clusterStateChangeReason.getCode();
            jsonWriter.name("Code");
            jsonWriter.value(code);
        }
        if (clusterStateChangeReason.getMessage() != null) {
            String message = clusterStateChangeReason.getMessage();
