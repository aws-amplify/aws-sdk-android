...
/**
 * JSON marshaller for POJO VolumeSpecification
 */
class VolumeSpecificationJsonMarshaller {

    public void marshall(VolumeSpecification volumeSpecification, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (volumeSpecification.getVolumeType() != null) {
            String volumeType = volumeSpecification.getVolumeType();
            jsonWriter.name("VolumeType");
            jsonWriter.value(volumeType);
        }
        if (volumeSpecification.getIops() != null) {
            Integer iops = volumeSpecification.getIops();
            jsonWriter.name("Iops");
            jsonWriter.value(iops);
        }
        if (volumeSpecification.getSizeInGB() != null) {
            Integer sizeInGB = volumeSpecification.getSizeInGB();
