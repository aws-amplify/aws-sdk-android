...
/**
 * JSON marshaller for POJO FieldToMatch
 */
class FieldToMatchJsonMarshaller {

    public void marshall(FieldToMatch fieldToMatch, AwsJsonWriter jsonWriter) throws Exception {
        jsonWriter.beginObject();
        if (fieldToMatch.getType() != null) {
            String type = fieldToMatch.getType();
            jsonWriter.name("Type");
            jsonWriter.value(type);
        }
        if (fieldToMatch.getData() != null) {
            String data = fieldToMatch.getData();
