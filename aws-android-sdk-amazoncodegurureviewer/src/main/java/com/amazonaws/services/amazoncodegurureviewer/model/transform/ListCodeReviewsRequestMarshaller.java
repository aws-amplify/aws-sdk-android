 ...
 */
public class ListCodeReviewsRequestMarshaller implements
        Marshaller<Request<ListCodeReviewsRequest>, ListCodeReviewsRequest> {
    ...
    public Request<ListCodeReviewsRequest> marshall(ListCodeReviewsRequest listCodeReviewsRequest) {
        if (listCodeReviewsRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(ListCodeReviewsRequest)");
        }

        Request<ListCodeReviewsRequest> request = new DefaultRequest<ListCodeReviewsRequest>(
                listCodeReviewsRequest, "AmazonCodeGuruReviewer");
        ...
        if (listCodeReviewsRequest.getProviderTypes() != null) {
            request.addParameter(
                    "ProviderTypes",
                    StringUtils.join(",",
                            listCodeReviewsRequest.getProviderTypes().toArray(new String[0])));
        }
        if (listCodeReviewsRequest.getStates() != null) {
            request.addParameter("States", StringUtils.join(",", listCodeReviewsRequest.getStates()
        ...
        if (listCodeReviewsRequest.getRepositoryNames() != null) {
            request.addParameter(
                    "RepositoryNames",
                    StringUtils.join(",",
                            listCodeReviewsRequest.getRepositoryNames().toArray(new String[0])));
        }
        ...
        if (listCodeReviewsRequest.getMaxResults() != null) {
            request.addParameter("MaxResults",
                    StringUtils.fromInteger(listCodeReviewsRequest.getMaxResults()));
        }
        if (listCodeReviewsRequest.getNextToken() != null) {
            request.addParameter("NextToken",
                    StringUtils.fromString(listCodeReviewsRequest.getNextToken()));
        }
