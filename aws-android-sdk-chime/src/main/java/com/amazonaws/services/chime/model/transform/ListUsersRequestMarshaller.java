...
public class ListUsersRequestMarshaller implements
    ...
    public Request<ListUsersRequest> marshall(ListUsersRequest listUsersRequest) {
        ...
        uriResourcePath = uriResourcePath.replace(
                "{accountId}",
                (listUsersRequest.getAccountId() == null) ? "" : StringUtils
                        .fromString(listUsersRequest.getAccountId()));
        if (listUsersRequest.getUserEmail() != null) {
            request.addParameter("user-email",
                    StringUtils.fromString(listUsersRequest.getUserEmail()));
        }
        if (listUsersRequest.getUserType() != null) {
            request.addParameter("user-type",
                    StringUtils.fromString(listUsersRequest.getUserType()));
        }
        if (listUsersRequest.getMaxResults() != null) {
            request.addParameter("max-results",
                    StringUtils.fromInteger(listUsersRequest.getMaxResults()));
        }
        if (listUsersRequest.getNextToken() != null) {
            request.addParameter("next-token",
                    StringUtils.fromString(listUsersRequest.getNextToken()));
        }
