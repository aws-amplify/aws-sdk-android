...
public class BatchCreateRoomMembershipResult implements Serializable {
    /**
     * <p>
     ...
     * @param errors <p>
     *            If the action fails for one or more of the member IDs in the
     *            request, a list of the member IDs is returned, along with
    ...
    public BatchCreateRoomMembershipResult withErrors(MemberError... errors) {
        if (getErrors() == null) {
            this.errors = new java.util.ArrayList<MemberError>(errors.length);
        }
        for (MemberError value : errors) {
