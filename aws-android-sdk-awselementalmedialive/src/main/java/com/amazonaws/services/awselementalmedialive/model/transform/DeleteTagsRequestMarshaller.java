...
public class DeleteTagsRequestMarshaller implements
    ...
    public Request<DeleteTagsRequest> marshall(DeleteTagsRequest deleteTagsRequest) {
        ...
        Request<DeleteTagsRequest> request = new DefaultRequest<DeleteTagsRequest>(
                deleteTagsRequest, "AWSElementalMediaLive");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/prod/tags/{resource-arn}";
        uriResourcePath = uriResourcePath.replace(
                "{resource-arn}",
                (deleteTagsRequest.getResourceArn() == null) ? "" : StringUtils
                        .fromString(deleteTagsRequest.getResourceArn()));
        if (deleteTagsRequest.getTagKeys() != null) {
            request.addParameter("tagKeys",
                    StringUtils.join(",", deleteTagsRequest.getTagKeys().toArray(new String[0])));
        }
