...
public class AndroidConnectivity implements Connectivity {
    ...
    // this method access constants that were added in the HONEYCOMB_MR2 release and is properly guarded from running on older devices.
    private void determineAvailability() {
        ...
        // default state
        hasWifi = false;
        // when we have connectivity manager, we assume we have some sort of connectivity
        hasMobile = cm != null;
        ...
        logger.v(String.format("Device Connectivity (%s)", hasWifi ? "On Wifi" : (hasMobile ? "On Mobile" : "No network connectivity")));
    }
