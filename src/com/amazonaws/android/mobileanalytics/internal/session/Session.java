 ...
 * A Session Object
 * 
 * Mostly immutable, with the exception of its stop time.
 * 
 * This session object tracks whether or not it has been paused by checking
 *   the status of it's stop time. A session's stop time is only set when the
 *   session has been paused, and is set to -1 if it is currently running.
 *   
 * Can be serialized and restored for persistence.
 * 
 */
public class Session implements JSONSerializable {
	...
	public String generateSessionID(final AnalyticsContext context) {
		...
		return StringUtil.trimOrPadString(uniqID, SESSION_ID_UNIQID_LENGTH, SESSION_ID_PAD_CHAR) + SESSION_ID_DELIMITER 
		     + time;
	}
