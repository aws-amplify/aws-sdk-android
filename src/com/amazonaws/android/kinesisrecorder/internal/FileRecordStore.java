...
import com.amazonaws.android.kinesisrecorder.KinesisRecorderConfig;
import com.amazonaws.android.kinesisrecorder.internal.logging.Logger;
...
public class FileRecordStore implements RecordStore {
	...
	public boolean put(final String record) {
		...
		try {
			...
			if(writer != null) {
				if(recordFile.length() + record.length() <= config.getMaxStorageSize()) {
					writer.write(record+"\n");
					writer.flush();
					success = true;
				} else {
