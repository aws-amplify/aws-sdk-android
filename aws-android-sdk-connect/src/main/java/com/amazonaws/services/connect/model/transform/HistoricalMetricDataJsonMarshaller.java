...
/**
 * JSON marshaller for POJO HistoricalMetricData
 */
class HistoricalMetricDataJsonMarshaller {

    public void marshall(HistoricalMetricData historicalMetricData, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (historicalMetricData.getMetric() != null) {
            HistoricalMetric metric = historicalMetricData.getMetric();
            jsonWriter.name("Metric");
            HistoricalMetricJsonMarshaller.getInstance().marshall(metric, jsonWriter);
        }
