...
/**
 * JSON request marshaller for DeleteRuleRequest
 */
    ...
    public Request<DeleteRuleRequest> marshall(DeleteRuleRequest deleteRuleRequest) {
        if (deleteRuleRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteRuleRequest)");
        }

        Request<DeleteRuleRequest> request = new DefaultRequest<DeleteRuleRequest>(
                deleteRuleRequest, "AmazonConnect");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/rules/{InstanceId}/{RuleId}";
        uriResourcePath = uriResourcePath.replace(
                "{InstanceId}",
                (deleteRuleRequest.getInstanceId() == null) ? "" : StringUtils
                        .fromString(deleteRuleRequest.getInstanceId()));
        uriResourcePath = uriResourcePath.replace(
                "{RuleId}",
                (deleteRuleRequest.getRuleId() == null) ? "" : StringUtils
                        .fromString(deleteRuleRequest.getRuleId()));
