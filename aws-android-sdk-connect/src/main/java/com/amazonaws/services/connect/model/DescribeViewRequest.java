 ...
 * <code>&lt;view-id&gt;:$SAVED</code>, which is either an actual version number
 * or an Amazon Connect managed qualifier <code>$SAVED | $LATEST</code>. If it
 * is not supplied, then <code>$LATEST</code> is assumed for customer managed
 * views and an error is returned if there is no published content available.
...
public class DescribeViewRequest extends AmazonWebServiceRequest implements Serializable {
     ...
     *
     * @return <p>
     *         The ViewId of the view. This must be an ARN for Amazon Web
