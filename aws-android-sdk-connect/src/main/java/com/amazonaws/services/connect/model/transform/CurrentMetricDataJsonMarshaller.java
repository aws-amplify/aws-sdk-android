...
/**
 * JSON marshaller for POJO CurrentMetricData
 */
class CurrentMetricDataJsonMarshaller {

    public void marshall(CurrentMetricData currentMetricData, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (currentMetricData.getMetric() != null) {
            CurrentMetric metric = currentMetricData.getMetric();
            jsonWriter.name("Metric");
            CurrentMetricJsonMarshaller.getInstance().marshall(metric, jsonWriter);
        }
