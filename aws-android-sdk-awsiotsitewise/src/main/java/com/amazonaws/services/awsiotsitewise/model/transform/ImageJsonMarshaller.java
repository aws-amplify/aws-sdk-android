...
class ImageJsonMarshaller {
    ...
    public void marshall(Image image, AwsJsonWriter jsonWriter) throws Exception {
        ...
        if (image.getLocationUrl() != null) {
            String locationUrl = image.getLocationUrl();
            jsonWriter.name("locationUrl");
            jsonWriter.value(locationUrl);
        }
        if (image.getLastUpdateDate() != null) {
            java.util.Date lastUpdateDate = image.getLastUpdateDate();
