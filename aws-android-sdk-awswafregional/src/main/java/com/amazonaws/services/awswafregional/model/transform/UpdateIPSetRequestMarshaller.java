...
public class UpdateIPSetRequestMarshaller implements
    ...
    public Request<UpdateIPSetRequest> marshall(UpdateIPSetRequest updateIPSetRequest) {
        ...
        Request<UpdateIPSetRequest> request = new DefaultRequest<UpdateIPSetRequest>(
                updateIPSetRequest, "AWSWAFRegional");
        ...
        try {
            ...
            if (updateIPSetRequest.getIPSetId() != null) {
                String iPSetId = updateIPSetRequest.getIPSetId();
                jsonWriter.name("IPSetId");
                jsonWriter.value(iPSetId);
            }
            if (updateIPSetRequest.getChangeToken() != null) {
                String changeToken = updateIPSetRequest.getChangeToken();
                jsonWriter.name("ChangeToken");
                jsonWriter.value(changeToken);
            }
            if (updateIPSetRequest.getUpdates() != null) {
                java.util.List<IPSetUpdate> updates = updateIPSetRequest.getUpdates();
                jsonWriter.name("Updates");
                jsonWriter.beginArray();
                for (IPSetUpdate updatesItem : updates) {
                    if (updatesItem != null) {
                        IPSetUpdateJsonMarshaller.getInstance().marshall(updatesItem, jsonWriter);
                    }
                }
                jsonWriter.endArray();
            }
