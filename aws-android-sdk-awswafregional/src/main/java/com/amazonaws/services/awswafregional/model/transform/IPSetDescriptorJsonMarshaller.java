...
/**
 * JSON marshaller for POJO IPSetDescriptor
 */
class IPSetDescriptorJsonMarshaller {

    public void marshall(IPSetDescriptor iPSetDescriptor, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (iPSetDescriptor.getType() != null) {
            String type = iPSetDescriptor.getType();
            jsonWriter.name("Type");
            jsonWriter.value(type);
        }
        if (iPSetDescriptor.getValue() != null) {
            String value = iPSetDescriptor.getValue();
