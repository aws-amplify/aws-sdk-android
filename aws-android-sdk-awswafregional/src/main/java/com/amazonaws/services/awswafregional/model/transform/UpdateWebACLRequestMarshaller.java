...
/**
 * JSON request marshaller for UpdateWebACLRequest
 */
    ...
    public Request<UpdateWebACLRequest> marshall(UpdateWebACLRequest updateWebACLRequest) {
        if (updateWebACLRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(UpdateWebACLRequest)");
        }

        Request<UpdateWebACLRequest> request = new DefaultRequest<UpdateWebACLRequest>(
                updateWebACLRequest, "AWSWAFRegional");
        ...
        try {
            ...
            if (updateWebACLRequest.getWebACLId() != null) {
                String webACLId = updateWebACLRequest.getWebACLId();
                jsonWriter.name("WebACLId");
                jsonWriter.value(webACLId);
            }
            if (updateWebACLRequest.getChangeToken() != null) {
                String changeToken = updateWebACLRequest.getChangeToken();
                jsonWriter.name("ChangeToken");
                jsonWriter.value(changeToken);
            }
            if (updateWebACLRequest.getUpdates() != null) {
                java.util.List<WebACLUpdate> updates = updateWebACLRequest.getUpdates();
                jsonWriter.name("Updates");
                jsonWriter.beginArray();
                for (WebACLUpdate updatesItem : updates) {
                    if (updatesItem != null) {
                        WebACLUpdateJsonMarshaller.getInstance().marshall(updatesItem, jsonWriter);
                    }
                }
                jsonWriter.endArray();
            }
            if (updateWebACLRequest.getDefaultAction() != null) {
                WafAction defaultAction = updateWebACLRequest.getDefaultAction();
