...
public class BucketConfigurationXmlFactory {
    ...
    public byte[] convertToXmlByteArray(BucketReplicationConfiguration replicationConfiguration) {
        ...
        final Map<String, ReplicationRule> rules = replicationConfiguration
                .getRules();

        final String role = replicationConfiguration.getRoleARN();
        xml.start("Role").value(role).end();
        for (final Map.Entry<String, ReplicationRule> entry : rules
                .entrySet()) {
    ...
    private void writeRule(XmlWriter xml, Rule rule) {
        ...
        if (hasCurrentExpirationPolicy(rule)) {
            // The rule attributes below are mutually exclusive, the service
            ...
            if (rule.getExpirationDate() != null) {
                xml.start("Date").value(ServiceUtils.formatIso8601Date(rule.getExpirationDate()))
                        .end();
            }
        ...
        if (rule.getAbortIncompleteMultipartUpload() != null) {
            ...
            xml.start("DaysAfterInitiation").value(Integer
                    .toString(rule.getAbortIncompleteMultipartUpload().getDaysAfterInitiation()))
                    .end();
    ...
    private class LifecyclePredicateVisitorImpl implements LifecyclePredicateVisitor {
        ...
        public void visit(LifecycleAndOperator lifecycleAndOperator) {
            xml.start("And");
            for (final LifecycleFilterPredicate predicate : lifecycleAndOperator.getOperands()) {
    ...
    private void writeInventoryDestination(XmlWriter xml, InventoryDestination destination) {
        ...
        xml.start("Destination");
        final InventoryS3BucketDestination s3BucketDestination = destination
    ...
    private void addInventoryOptionalFields(XmlWriter xml, List<String> optionalFields) {
        if (isNullOrEmpty(optionalFields)) {
            return;
        }
    ...
    private void writeRule(XmlWriter xml, TagSet tagset) {
        xml.start("TagSet");
        for (final String key : tagset.getAllTags().keySet()) {
            xml.start("Tag");
    ...
    private void writeStorageClassAnalysis(XmlWriter xml,
    ...
    private void writeAnalyticsExportDestination(XmlWriter xml,
        ...
        if (destination.getS3BucketDestination() != null) {
            xml.start("S3BucketDestination");
            final AnalyticsS3BucketDestination s3BucketDestination = destination
                    .getS3BucketDestination();
            addParameterIfNotNull(xml, "Format", s3BucketDestination.getFormat());
    ...
    private class AnalyticsPredicateVisitorImpl implements AnalyticsPredicateVisitor {
        ...
        public void visit(AnalyticsAndOperator analyticsAndOperator) {
            xml.start("And");
            for (final AnalyticsFilterPredicate predicate : analyticsAndOperator.getOperands()) {
                predicate.accept(this);
            }
            xml.end();
        }
    ...
    private class MetricsPredicateVisitorImpl implements MetricsPredicateVisitor {
        ...
        public void visit(MetricsAndOperator metricsAndOperator) {
            xml.start("And");
            for (final MetricsFilterPredicate predicate : metricsAndOperator.getOperands()) {
                predicate.accept(this);
            }
            xml.end();
        }
    ...
    private void writePrefix(XmlWriter xml, Rule rule) {
        // If no filter is set stick with the legacy behavior where we treat a
        // null prefix as empty prefix.
