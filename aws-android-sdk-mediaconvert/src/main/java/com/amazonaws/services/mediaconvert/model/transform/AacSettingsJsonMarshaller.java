...
class AacSettingsJsonMarshaller {
    ...
    public void marshall(AacSettings aacSettings, AwsJsonWriter jsonWriter) throws Exception {
        ...
        if (aacSettings.getAudioDescriptionBroadcasterMix() != null) {
            String audioDescriptionBroadcasterMix = aacSettings.getAudioDescriptionBroadcasterMix();
            jsonWriter.name("AudioDescriptionBroadcasterMix");
            jsonWriter.value(audioDescriptionBroadcasterMix);
        }
        if (aacSettings.getBitrate() != null) {
            Integer bitrate = aacSettings.getBitrate();
            jsonWriter.name("Bitrate");
            jsonWriter.value(bitrate);
        }
        if (aacSettings.getCodecProfile() != null) {
            String codecProfile = aacSettings.getCodecProfile();
        ...
        if (aacSettings.getSampleRate() != null) {
            Integer sampleRate = aacSettings.getSampleRate();
            jsonWriter.name("SampleRate");
            jsonWriter.value(sampleRate);
        }
        if (aacSettings.getSpecification() != null) {
            String specification = aacSettings.getSpecification();
            jsonWriter.name("Specification");
            jsonWriter.value(specification);
        }
