...
public class SessionObjectTest extends MobileAnalyticsTestBase {
    ...
    public void setup() {
        ...
        when(mockAnalyticsContext.getUniqueId()).thenReturn(Id.valueOf(DEF_UNIQ_ID));

        target = Session.newInstance(mockAnalyticsContext);
    }
    ...
    public void sessionID_padsUniqueID_ifNullorEmpty() {
        ...
        when(mockAnalyticsContext.getUniqueId()).thenReturn(Id.valueOf(null));
        target = Session.newInstance(mockAnalyticsContext);
        assertTrue(pattern.matcher(target.getSessionID()).find());

        when(mockAnalyticsContext.getUniqueId()).thenReturn(Id.valueOf(""));
        target = Session.newInstance(mockAnalyticsContext);
        assertTrue(pattern.matcher(target.getSessionID()).find());
    }
