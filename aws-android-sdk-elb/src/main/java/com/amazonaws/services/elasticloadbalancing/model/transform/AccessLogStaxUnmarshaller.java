...
/**
 * StAX unmarshaller for model AccessLog
 */
class AccessLogStaxUnmarshaller implements Unmarshaller<AccessLog, StaxUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (xmlEvent == XmlPullParser.START_TAG) {
                ...
                if (context.testExpression("S3BucketName", targetDepth)) {
                    accessLog.setS3BucketName(StringStaxUnmarshaller.getInstance().unmarshall(
                            context));
                    continue;
                }
                if (context.testExpression("EmitInterval", targetDepth)) {
                    accessLog.setEmitInterval(IntegerStaxUnmarshaller.getInstance().unmarshall(
                            context));
                    continue;
                }
                if (context.testExpression("S3BucketPrefix", targetDepth)) {
                    accessLog.setS3BucketPrefix(StringStaxUnmarshaller.getInstance().unmarshall(
                            context));
                    continue;
                }
