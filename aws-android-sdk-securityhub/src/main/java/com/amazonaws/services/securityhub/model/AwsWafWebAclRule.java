...
public class AwsWafWebAclRule implements Serializable {
     ...
     * with a lower value for <code>Priority</code> are evaluated before rules
     * with a higher value. The value must be a unique integer. If you add
     * multiple rules to a WebACL, the values do not need to be consecutive.
     * </p>
     */
     ...
     *            evaluated. Rules with a lower value for <code>Priority</code>
     *            are evaluated before rules with a higher value. The value must
     *            be a unique integer. If you add multiple rules to a WebACL,
