...
class SpekeKeyProviderJsonMarshaller {
    ...
    public void marshall(SpekeKeyProvider spekeKeyProvider, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (spekeKeyProvider.getCertificateArn() != null) {
            String certificateArn = spekeKeyProvider.getCertificateArn();
            jsonWriter.name("CertificateArn");
            jsonWriter.value(certificateArn);
        }
        if (spekeKeyProvider.getResourceId() != null) {
            String resourceId = spekeKeyProvider.getResourceId();
    ...
    public static SpekeKeyProviderJsonMarshaller getInstance() {
        if (instance == null)
            instance = new SpekeKeyProviderJsonMarshaller();
        return instance;
    }
}
