...
class AudioNormalizationSettingsJsonMarshaller {
    ...
    public void marshall(AudioNormalizationSettings audioNormalizationSettings,
        ...
        if (audioNormalizationSettings.getCorrectionGateLevel() != null) {
            Integer correctionGateLevel = audioNormalizationSettings.getCorrectionGateLevel();
            jsonWriter.name("CorrectionGateLevel");
            jsonWriter.value(correctionGateLevel);
        }
        if (audioNormalizationSettings.getLoudnessLogging() != null) {
            String loudnessLogging = audioNormalizationSettings.getLoudnessLogging();
            jsonWriter.name("LoudnessLogging");
            jsonWriter.value(loudnessLogging);
        }
        if (audioNormalizationSettings.getPeakCalculation() != null) {
            String peakCalculation = audioNormalizationSettings.getPeakCalculation();
            jsonWriter.name("PeakCalculation");
            jsonWriter.value(peakCalculation);
        }
