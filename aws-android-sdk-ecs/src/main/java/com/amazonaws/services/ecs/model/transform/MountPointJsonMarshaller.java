...
/**
 * JSON marshaller for POJO MountPoint
 */
class MountPointJsonMarshaller {

    public void marshall(MountPoint mountPoint, AwsJsonWriter jsonWriter) throws Exception {
        jsonWriter.beginObject();
        if (mountPoint.getSourceVolume() != null) {
            String sourceVolume = mountPoint.getSourceVolume();
            jsonWriter.name("sourceVolume");
            jsonWriter.value(sourceVolume);
        }
        if (mountPoint.getContainerPath() != null) {
            String containerPath = mountPoint.getContainerPath();
            jsonWriter.name("containerPath");
            jsonWriter.value(containerPath);
        }
        if (mountPoint.getReadOnly() != null) {
            Boolean readOnly = mountPoint.getReadOnly();
