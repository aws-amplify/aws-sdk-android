...
/**
 * JSON marshaller for POJO PlacementStrategy
 */
class PlacementStrategyJsonMarshaller {

    public void marshall(PlacementStrategy placementStrategy, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (placementStrategy.getType() != null) {
            String type = placementStrategy.getType();
            jsonWriter.name("type");
            jsonWriter.value(type);
        }
        if (placementStrategy.getField() != null) {
            String field = placementStrategy.getField();
