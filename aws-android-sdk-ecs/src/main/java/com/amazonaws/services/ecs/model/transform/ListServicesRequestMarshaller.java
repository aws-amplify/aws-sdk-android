...
/**
 * JSON request marshaller for ListServicesRequest
 */
    ...
    public Request<ListServicesRequest> marshall(ListServicesRequest listServicesRequest) {
        if (listServicesRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(ListServicesRequest)");
        }

        Request<ListServicesRequest> request = new DefaultRequest<ListServicesRequest>(
                listServicesRequest, "AmazonEC2ContainerService");
        ...
        try {
            ...
            if (listServicesRequest.getCluster() != null) {
                String cluster = listServicesRequest.getCluster();
                jsonWriter.name("cluster");
                jsonWriter.value(cluster);
            }
            if (listServicesRequest.getNextToken() != null) {
                String nextToken = listServicesRequest.getNextToken();
                jsonWriter.name("nextToken");
                jsonWriter.value(nextToken);
            }
            if (listServicesRequest.getMaxResults() != null) {
                Integer maxResults = listServicesRequest.getMaxResults();
                jsonWriter.name("maxResults");
                jsonWriter.value(maxResults);
            }
            if (listServicesRequest.getLaunchType() != null) {
                String launchType = listServicesRequest.getLaunchType();
                jsonWriter.name("launchType");
                jsonWriter.value(launchType);
            }
            if (listServicesRequest.getSchedulingStrategy() != null) {
                String schedulingStrategy = listServicesRequest.getSchedulingStrategy();
