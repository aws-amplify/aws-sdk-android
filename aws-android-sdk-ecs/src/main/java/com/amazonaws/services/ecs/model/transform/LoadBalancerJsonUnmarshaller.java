...
class LoadBalancerJsonUnmarshaller implements Unmarshaller<LoadBalancer, JsonUnmarshallerContext> {
    ...
    public LoadBalancer unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (reader.hasNext()) {
            ...
            if (name.equals("targetGroupArn")) {
                loadBalancer.setTargetGroupArn(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("loadBalancerName")) {
                loadBalancer.setLoadBalancerName(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("containerName")) {
                loadBalancer.setContainerName(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("containerPort")) {
                loadBalancer.setContainerPort(IntegerJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else {
                reader.skipValue();
            }
