 ...
 * for any manual DB snapshots that contain private information that you don't
 * want available to all AWS accounts. If the manual DB snapshot is encrypted,
 * it can be shared, but only by specifying a list of authorized AWS account IDs
 * for the <code>ValuesToAdd</code> parameter. You can't use <code>all</code> as
