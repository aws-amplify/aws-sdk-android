...
public class RegisterTypeRequest extends AmazonWebServiceRequest implements Serializable {
     ...
     *         to assume when invoking the resource provider. If your resource
     *         type calls AWS APIs in any of its handlers, you must create an
     *         <i> <a href=
     ...
     *         resource provider handler, CloudFormation assumes this execution
     *         role to create a temporary session token, which it then passes to
     *         the resource provider handler, thereby supplying your resource
     ...
     *            The Amazon Resource Name (ARN) of the IAM role for
     *            CloudFormation to assume when invoking the resource provider.
     *            If your resource type calls AWS APIs in any of its handlers,
     *            you must create an <i> <a href=
     *            "https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html"
     *            >IAM execution role</a> </i> that includes the necessary
     *            permissions to call those AWS APIs, and provision that
     *            execution role in your account. When CloudFormation needs to
     *            invoke the resource provider handler, CloudFormation assumes
     *            this execution role to create a temporary session token, which
     *            it then passes to the resource provider handler, thereby
     *            supplying your resource provider with the appropriate
     *            credentials.
     *            </p>
     */
