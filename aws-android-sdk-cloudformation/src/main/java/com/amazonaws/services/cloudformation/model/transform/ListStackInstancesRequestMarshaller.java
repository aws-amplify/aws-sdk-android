...
public class ListStackInstancesRequestMarshaller implements
    ...
    public Request<ListStackInstancesRequest> marshall(
        ...
        if (listStackInstancesRequest.getFilters() != null) {
            prefix = "Filters";
            java.util.List<StackInstanceFilter> filters = listStackInstancesRequest.getFilters();
            int filtersIndex = 1;
            String filtersPrefix = prefix;
            for (StackInstanceFilter filtersItem : filters) {
                prefix = filtersPrefix + ".member." + filtersIndex;
                if (filtersItem != null) {
                    StackInstanceFilterStaxMarshaller.getInstance().marshall(filtersItem, request,
                            prefix + ".");
                }
                filtersIndex++;
            }
