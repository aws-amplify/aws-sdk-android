...
/**
 * JSON marshaller for POJO ContextDataType
 */
class ContextDataTypeJsonMarshaller {

    public void marshall(ContextDataType contextDataType, AwsJsonWriter jsonWriter)
            throws Exception {
        ...
        if (contextDataType.getHttpHeaders() != null) {
            java.util.List<HttpHeader> httpHeaders = contextDataType.getHttpHeaders();
            jsonWriter.name("HttpHeaders");
            jsonWriter.beginArray();
            for (HttpHeader httpHeadersItem : httpHeaders) {
                if (httpHeadersItem != null) {
