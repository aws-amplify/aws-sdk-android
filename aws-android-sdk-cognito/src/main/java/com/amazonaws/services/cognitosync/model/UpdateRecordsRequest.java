 ...
 * and you try and submit a new highScore with sync count of 4, the request will
 * be rejected. To obtain the current sync count for a record, call ListRecords.
 * On a successful update of the record, the response returns the new sync count
 * for that record. You should present that sync count the next time you try to
 * update that same record. When the record does not exist, specify the sync
 * count as 0.
 * </p>
 * <p>
...
public class UpdateRecordsRequest extends AmazonWebServiceRequest implements Serializable {
     ...
     */
    private java.util.List<RecordPatch> recordPatches;
    ...
    public void setRecordPatches(java.util.Collection<RecordPatch> recordPatches) {
        ...
        this.recordPatches = new java.util.ArrayList<RecordPatch>(recordPatches);
    }
    ...
    public UpdateRecordsRequest withRecordPatches(RecordPatch... recordPatches) {
        if (getRecordPatches() == null) {
            this.recordPatches = new java.util.ArrayList<RecordPatch>(recordPatches.length);
        }
        for (RecordPatch value : recordPatches) {
            this.recordPatches.add(value);
        }
        return this;
    }
     ...
     * dataset and identity.
     *
     * @param syncSessionToken The SyncSessionToken returned by a previous call
     *            to ListRecords for this dataset and identity.
     */
     ...
     * implemented.
     *
     * @param clientContext Intended to supply a device ID that will populate
     *            the lastModifiedBy field referenced in other methods. The
     *            ClientContext field is not yet implemented.
     */
    ...
    public int hashCode() {
        ...
        hashCode = prime * hashCode
                + ((getRecordPatches() == null) ? 0 : getRecordPatches().hashCode());
        hashCode = prime * hashCode
