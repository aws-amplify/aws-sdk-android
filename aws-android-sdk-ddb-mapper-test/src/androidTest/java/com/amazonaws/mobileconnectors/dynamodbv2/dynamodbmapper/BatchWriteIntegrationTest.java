...
public class BatchWriteIntegrationTest extends DynamoDBMapperIntegrationTestBase {
    ...
    public void testBatchSaveAsArray() throws Exception {
        ...
        DynamoDBMapper mapper = new DynamoDBMapper(dynamo);
        NumberSetAttributeClass[] objsArray = objs
                .toArray(new NumberSetAttributeClass[objs.size()]);
    ...
    public void testMultipleTables() throws Exception {
        ...
        for (Object obj : objs) {
            ...
            if (obj instanceof NumberSetAttributeClass) {
                loaded = mapper.load(NumberSetAttributeClass.class,
                        ((NumberSetAttributeClass) obj).getKey());
    ...
    /**
     * Test whether it finish processing all the items even if the first batch
     * is failed.
     */
    @Test
    public void testErrorHandling() {
        ...
        assertEquals(numItems, failedBatches.get(0).getUnprocessedItems().get("tableNotExist")
                .size());

        // The second batch succeeds, get them back
        for (Object obj : objs.subList(25, 50)) {
            RangeKeyClass loaded = mapper.load(RangeKeyClass.class, ((RangeKeyClass) obj).getKey(),
                    ((RangeKeyClass) obj).getRangeKey());
            assertEquals(obj, loaded);
        }
    }
    ...
    public void testLargeRequestEntity() {
        ...
        for (BinaryAttributeByteBufferClass obj : objs) {
            BinaryAttributeByteBufferClass loaded = mapper.load(
                    BinaryAttributeByteBufferClass.class, obj.getKey());
            assertEquals(obj, loaded);
        }
