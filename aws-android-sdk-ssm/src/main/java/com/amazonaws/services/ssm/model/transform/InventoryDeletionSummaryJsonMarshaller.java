...
class InventoryDeletionSummaryJsonMarshaller {
    ...
    public void marshall(InventoryDeletionSummary inventoryDeletionSummary, AwsJsonWriter jsonWriter)
            throws Exception {
        ...
        if (inventoryDeletionSummary.getSummaryItems() != null) {
            java.util.List<InventoryDeletionSummaryItem> summaryItems = inventoryDeletionSummary
                    .getSummaryItems();
            jsonWriter.name("SummaryItems");
            jsonWriter.beginArray();
            for (InventoryDeletionSummaryItem summaryItemsItem : summaryItems) {
                if (summaryItemsItem != null) {
                    InventoryDeletionSummaryItemJsonMarshaller.getInstance().marshall(
                            summaryItemsItem, jsonWriter);
                }
            }
            jsonWriter.endArray();
        }
    ...
    public static InventoryDeletionSummaryJsonMarshaller getInstance() {
        if (instance == null)
            instance = new InventoryDeletionSummaryJsonMarshaller();
        return instance;
    }
}
