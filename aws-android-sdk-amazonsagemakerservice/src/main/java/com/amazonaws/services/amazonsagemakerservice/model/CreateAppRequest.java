 ...
 * Amazon SageMaker Amazon SageMaker Studio (Studio) upon access to the
 * associated Studio Domain, and when new kernel configurations are selected by
 * the user. A user may have multiple Apps active simultaneously. Apps will
 * automatically terminate and be deleted when stopped from within Studio, or
 * when the DeleteApp API is manually called. UserProfiles are limited to 5
