...
class ResourceSpecJsonMarshaller {
    ...
    public void marshall(ResourceSpec resourceSpec, AwsJsonWriter jsonWriter) throws Exception {
        ...
        if (resourceSpec.getEnvironmentArn() != null) {
            String environmentArn = resourceSpec.getEnvironmentArn();
            jsonWriter.name("EnvironmentArn");
            jsonWriter.value(environmentArn);
        }
