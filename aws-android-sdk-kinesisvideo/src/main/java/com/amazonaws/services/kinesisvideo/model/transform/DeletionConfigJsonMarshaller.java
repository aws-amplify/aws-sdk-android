...
/**
 * JSON marshaller for POJO DeletionConfig
 */
class DeletionConfigJsonMarshaller {

    public void marshall(DeletionConfig deletionConfig, AwsJsonWriter jsonWriter) throws Exception {
        jsonWriter.beginObject();
        if (deletionConfig.getEdgeRetentionInHours() != null) {
            Integer edgeRetentionInHours = deletionConfig.getEdgeRetentionInHours();
            jsonWriter.name("EdgeRetentionInHours");
            jsonWriter.value(edgeRetentionInHours);
        }
        if (deletionConfig.getLocalSizeConfig() != null) {
            LocalSizeConfig localSizeConfig = deletionConfig.getLocalSizeConfig();
            jsonWriter.name("LocalSizeConfig");
            LocalSizeConfigJsonMarshaller.getInstance().marshall(localSizeConfig, jsonWriter);
        }
        if (deletionConfig.getDeleteAfterUpload() != null) {
            Boolean deleteAfterUpload = deletionConfig.getDeleteAfterUpload();
            jsonWriter.name("DeleteAfterUpload");
            jsonWriter.value(deleteAfterUpload);
        }
