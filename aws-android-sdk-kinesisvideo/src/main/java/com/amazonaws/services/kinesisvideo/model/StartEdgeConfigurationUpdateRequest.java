...
/**
 * <p>
 * An asynchronous API that updates a stream’s existing edge configuration. If
 ...
 * Edge Agent IoT Greengrass component that runs on an IoT Hub Device setup at
 * your premise. The time to sync can vary and depends on the connectivity of
 * the Hub Device. The <code>SyncStatus</code> will be updated as the edge
 * configuration is acknowledged, and synced with the Edge Agent. You will have
 * to wait for the sync status to reach a terminal state such as:
 * <code>IN_SYNC</code> and <code>SYNC_FAILED</code>, before using this API
 * again.
 * </p>
 * <p>
 * If you invoke this API during the syncing process, a
 * <code>ResourceInUseException</code> will be thrown. The connectivity of the
