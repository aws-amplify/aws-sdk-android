...
class ChangeStaxMarshaller {
    ...
    public void marshall(Change _change, Request<?> request, String _prefix) {
        String prefix;
        if (_change.getAction() != null) {
            prefix = _prefix + "Action";
            String action = _change.getAction();
            request.addParameter(prefix, StringUtils.fromString(action));
        }
        if (_change.getResourceRecordSet() != null) {
            prefix = _prefix + "ResourceRecordSet";
            ResourceRecordSet resourceRecordSet = _change.getResourceRecordSet();
            ResourceRecordSetStaxMarshaller.getInstance().marshall(resourceRecordSet, request,
                    prefix + ".");
        }
    }
