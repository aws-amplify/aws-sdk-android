...
/**
 * StAX request marshaller for DeleteHostedZoneRequest
 */
    ...
    public Request<DeleteHostedZoneRequest> marshall(DeleteHostedZoneRequest deleteHostedZoneRequest) {
        if (deleteHostedZoneRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteHostedZoneRequest)");
        }

        Request<DeleteHostedZoneRequest> request = new DefaultRequest<DeleteHostedZoneRequest>(
                deleteHostedZoneRequest, "AmazonRoute53");
        request.addParameter("Action", "DeleteHostedZone");
        request.addParameter("Version", "2013-04-01");

        String prefix;
        if (deleteHostedZoneRequest.getId() != null) {
            prefix = "Id";
            String id = deleteHostedZoneRequest.getId();
