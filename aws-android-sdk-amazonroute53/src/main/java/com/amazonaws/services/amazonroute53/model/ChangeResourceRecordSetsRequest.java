 ...
 * For example, you can use <code>ChangeResourceRecordSets</code> to create a
 * resource record set that routes traffic for test.example.com to a web server
 * that has an IP address of 192.0.2.44.
 * </p>
 * <p>
 ...
 * request and then either makes all or none of the changes in the change batch
 * request. This ensures that DNS routing isn't adversely affected by partial
 ...
 * the <code>CNAME</code> resource record set for www.example.com and creates an
 * alias resource record set for www.example.com. If validation for both records
 ...
 * the <code>DELETE</code> or the <code>CREATE</code> action fails, then the
 * request is canceled, and the original <code>CNAME</code> record continues to
 * exist.
 * </p>
 * <note>
 * <p>
 * If you try to delete the same resource record set more than once in a single
 * change batch, Route 53 returns an <code>InvalidChangeBatch</code> error.
 * </p>
 * </note>
 * <p>
 * <b>Traffic Flow</b>
 ...
 * traffic policy, then associate the traffic policy with one or more domain
 * names (such as example.com) or subdomain names (such as www.example.com), in
 * the same hosted zone or in multiple hosted zones. You can roll back the
 * updates if the new configuration isn't performing as expected. For more
 ...
 * creates it. If a resource set does exist, Route 53 updates it with the values
 * in the request.
 * </p>
 * </li>
 * </ul>
 ...
 * want to create, delete, or update, such as weighted, alias, or failover. The
 * XML elements in your request must appear in the order listed in the syntax.
 * </p>
 * <p>
 ...
 * create, delete, or update by using <code>ChangeResourceRecordSets</code>.
 * </p>
 * <p>
 ...
 * While your changes are propagating, <code>GetChange</code> returns a status
 * of <code>PENDING</code>. When propagation is complete, <code>GetChange</code>
 * returns a status of <code>INSYNC</code>. Changes generally propagate to all
 * Route 53 name servers within 60 seconds. For more information, see <a href=
 ...
 * </p>
 * <p>
 * For information about the limits on a <code>ChangeResourceRecordSets</code>
 * request, see <a href=
