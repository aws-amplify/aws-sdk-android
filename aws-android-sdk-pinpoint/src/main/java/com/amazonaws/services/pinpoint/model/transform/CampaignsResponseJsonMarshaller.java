...
/**
 * JSON marshaller for POJO CampaignsResponse
 */
class CampaignsResponseJsonMarshaller {

    public void marshall(CampaignsResponse campaignsResponse, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (campaignsResponse.getItem() != null) {
            java.util.List<CampaignResponse> item = campaignsResponse.getItem();
            jsonWriter.name("Item");
            jsonWriter.beginArray();
            for (CampaignResponse itemItem : item) {
                if (itemItem != null) {
                    CampaignResponseJsonMarshaller.getInstance().marshall(itemItem, jsonWriter);
                }
            }
            jsonWriter.endArray();
        }
