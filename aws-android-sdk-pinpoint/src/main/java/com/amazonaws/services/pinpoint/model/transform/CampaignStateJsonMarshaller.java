...
/**
 * JSON marshaller for POJO CampaignState
 */
class CampaignStateJsonMarshaller {

    public void marshall(CampaignState campaignState, AwsJsonWriter jsonWriter) throws Exception {
        jsonWriter.beginObject();
        if (campaignState.getCampaignStatus() != null) {
            String campaignStatus = campaignState.getCampaignStatus();
            jsonWriter.name("CampaignStatus");
            jsonWriter.value(campaignStatus);
        }
