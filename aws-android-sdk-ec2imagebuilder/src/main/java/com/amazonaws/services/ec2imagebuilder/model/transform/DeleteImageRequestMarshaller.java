...
/**
 * JSON request marshaller for DeleteImageRequest
 */
    ...
    public Request<DeleteImageRequest> marshall(DeleteImageRequest deleteImageRequest) {
        if (deleteImageRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteImageRequest)");
        }

        Request<DeleteImageRequest> request = new DefaultRequest<DeleteImageRequest>(
                deleteImageRequest, "EC2ImageBuilder");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/DeleteImage";
        if (deleteImageRequest.getImageBuildVersionArn() != null) {
            request.addParameter("imageBuildVersionArn",
                    StringUtils.fromString(deleteImageRequest.getImageBuildVersionArn()));
        }
