...
class LocationJsonUnmarshaller implements Unmarshaller<Location, JsonUnmarshallerContext> {
    ...
    public Location unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (reader.hasNext()) {
            ...
            if (name.equals("Jdbc")) {
                location.setJdbc(new ListUnmarshaller<CodeGenNodeArg>(
                        CodeGenNodeArgJsonUnmarshaller.getInstance()
                        )
                                .unmarshall(context));
            } else if (name.equals("S3")) {
                location.setS3(new ListUnmarshaller<CodeGenNodeArg>(CodeGenNodeArgJsonUnmarshaller
                        .getInstance()
                        )
                                .unmarshall(context));
            } else if (name.equals("DynamoDB")) {
                location.setDynamoDB(new ListUnmarshaller<CodeGenNodeArg>(
                        CodeGenNodeArgJsonUnmarshaller.getInstance()
                        )
                                .unmarshall(context));
            } else {
