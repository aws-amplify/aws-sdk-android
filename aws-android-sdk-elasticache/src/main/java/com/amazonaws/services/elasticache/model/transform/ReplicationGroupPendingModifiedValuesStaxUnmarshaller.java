...
/**
 * StAX unmarshaller for model ReplicationGroupPendingModifiedValues
 */
class ReplicationGroupPendingModifiedValuesStaxUnmarshaller implements
        Unmarshaller<ReplicationGroupPendingModifiedValues, StaxUnmarshallerContext> {

    public ReplicationGroupPendingModifiedValues unmarshall(StaxUnmarshallerContext context)
            throws Exception {
            ...
            if (xmlEvent == XmlPullParser.START_TAG) {
                if (context.testExpression("PrimaryClusterId", targetDepth)) {
                    replicationGroupPendingModifiedValues
                            .setPrimaryClusterId(StringStaxUnmarshaller.getInstance().unmarshall(
                                    context));
                    continue;
                }
                if (context.testExpression("AutomaticFailoverStatus", targetDepth)) {
                    replicationGroupPendingModifiedValues
                            .setAutomaticFailoverStatus(StringStaxUnmarshaller.getInstance()
                                    .unmarshall(context));
                    continue;
                }
                if (context.testExpression("Resharding", targetDepth)) {
                    replicationGroupPendingModifiedValues
                            .setResharding(ReshardingStatusStaxUnmarshaller.getInstance()
                                    .unmarshall(context));
                    continue;
                }
                if (context.testExpression("AuthTokenStatus", targetDepth)) {
                    replicationGroupPendingModifiedValues.setAuthTokenStatus(StringStaxUnmarshaller
