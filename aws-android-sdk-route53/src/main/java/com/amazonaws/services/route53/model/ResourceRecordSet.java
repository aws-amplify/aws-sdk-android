...
public class ResourceRecordSet implements Serializable {
    /**
     * <p>
     * For <code>ChangeResourceRecordSets</code> requests, the name of the
     * record that you want to create, update, or delete. For
     * <code>ListResourceRecordSets</code> responses, the name of a record in
     ...
     * name that you specify is fully qualified. This means that Route 53 treats
     ...
     * to route traffic to your distribution, one with a value of <code>A</code>
     * and one with a value of <code>AAAA</code>.
     * </p>
     * </li>
     * <li>
     * <p>
     ...
     * are supported except <code>NS</code> and <code>SOA</code>.
     * </p>
     * <note>
     * <p>
     ...
     * which the value of <code>Type</code> is <code>CNAME</code>. This is
     ...
     * weighted resource record sets named acme.example.com that have a type of
     * A. In a group of resource record sets that have the same name and type,
     * the value of <code>SetIdentifier</code> must be unique for each resource
     * record set.
     * </p>
     * <p>
     ...
     * using the current resource record set. Route 53 calculates the sum of the
     ...
     * record set. However, if you set <code>Weight</code> to <code>0</code> for
     * all resource record sets that have the same combination of DNS name and
     ...
     * >Options for Configuring Route 53 Active-Active and Active-Passive
     ...
     * where you created the resource that this resource record set refers to.
     * The resource typically is an AWS resource, such as an EC2 instance or an
     ...
     * user and the associated Amazon EC2 Region. Route 53 then returns the
     ...
     * ap-northeast-3, eu-north-1, sa-east-1, cn-north-1, cn-northwest-1,
     * ap-east-1, me-south-1, ap-south-1, af-south-1, eu-south-1
     */
     ...
     * We recommend that you create a resource record set for which the value of
     * <code>CountryCode</code> is <code>*</code>. Two groups of queries are
     * routed to the resource that you specify in this record: queries that come
     * from locations for which you haven't created geolocation resource record
     * sets and queries from IP addresses that aren't mapped to a location. If
     * you don't create a <code>*</code> resource record set, Route 53 returns a
     ...
     * the secondary resource record set. This is true regardless of the health
     ...
     * <li>
     * <p>
     * When all records are unhealthy, Route 53 responds to DNS queries with up
     ...
     * healthy, include the <code>HealthCheckId</code> element and specify the
     ...
     * <code>HealthCheckId</code> element to a resource record set, Route 53
     ...
     * </p>
     * <p>
     * Specifying a value for <code>HealthCheckId</code> is useful only when
     * Route 53 is choosing between two or more resource record sets to respond
     * to a DNS query, and you want Route 53 to base the choice in part on the
     * status of a health check. Configuring health checks makes sense only in
     * the following configurations:
     * </p>
     * <ul>
     * <li>
     * <p>
     * <b>Non-alias resource record sets</b>: You're checking the health of a
     ...
     * 53 stops responding to DNS queries using the value for that resource
     * record set.
     * </p>
     * <p>
     ...
     * resource record set that has <code>*</code> for <code>CountryCode</code>
     * is <code>*</code>, which applies to all locations. If the endpoint for
     * the state resource record set is unhealthy, Route 53 checks for healthy
     * resource record sets in the following order until it finds a resource
     ...
     * server (such as <code>us-east-2-www.example.com</code>), not the name of
     * the resource record sets (<code>www.example.com</code>).
     * </p>
     * <important>
     * <p>
     ...
     * creates a resource record set. <code>TrafficPolicyInstanceId</code> is
     * the ID of the traffic policy instance that Route 53 created this resource
     * record set for.
     * </p>
     * <important>
     * <p>
     * To delete the resource record set that is associated with a traffic
     * policy instance, use <code>DeleteTrafficPolicyInstance</code>. Route 53
     * will delete the resource record set automatically. If you delete the
     * resource record set by using <code>ChangeResourceRecordSets</code>, Route
     ...
     *
     * @return <p>
     *         For <code>ChangeResourceRecordSets</code> requests, the name of
     *         the record that you want to create, update, or delete. For
     ...
     *         This means that Route 53 treats <code>www.example.com</code>
     *         (without a trailing dot) and <code>www.example.com.</code> (with
     ...
     *         name, for example, <code>*.example.com</code>. You can't use an *
     ...
     *
     * @param name <p>
     *            For <code>ChangeResourceRecordSets</code> requests, the name
     ...
     *            <p>
     *            You can use the asterisk (*) wildcard to replace the leftmost
     *            label in a domain name, for example,
     ...
     *            <li>
     *            <p>
     *            If you include * in any position other than the leftmost label
     *            in a domain name, DNS treats it as an * character (ASCII 42),
     *            not as a wildcard.
     *            </p>
     *            <important>
     *            <p>
     ...
     *            <p>
     *            You can use the * wildcard as the leftmost label in a domain
     *            name, for example, <code>*.example.com</code>. You can't use
     *            an * for one of the middle labels, for example,
     *            <code>marketing.*.example.com</code>. In addition, the * must
     *            replace the entire label; for example, you can't specify
     *            <code>prod*.example.com</code>.
     *            </p>
     ...
     *         <code>SRV</code> | <code>TXT</code>. When creating a group of
     ...
     *         <li>
     *         <p>
     *         <b>Amazon API Gateway custom regional APIs and edge-optimized
     *         APIs:</b> <code>A</code>
     *         </p>
     *         </li>
     *         <li>
     ...
     *         value of <code>A</code> and one with a value of <code>AAAA</code>
     *         .
     *         </p>
     *         </li>
     *         <li>
     ...
     *
     * @param type <p>
     *            The DNS record type. For information about different record
     *            types and how data is encoded for them, see <a href=
     *            "https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/ResourceRecordTypes.html"
     ...
     *            sets, specify the same value for all of the resource record
     *            sets in the group.
     *            </p>
     *            <p>
     *            Valid values for multivalue answer resource record sets:
     ...
     *            sender of email messages. However, we no longer recommend that
     *            you create resource record sets for which the value of
     *            <code>Type</code> is <code>SPF</code>. RFC 7208, <i>Sender
     *            Policy Framework (SPF) for Authorizing Use of Domains in
     *            Email, Version 1</i>, has been updated to say,
     *            "...[I]ts existence and mechanism defined in [RFC4408] have led to some interoperability issues. Accordingly, its use is no longer appropriate for SPF version 1; implementations are not to use it."
     *            In RFC 7208, see section 14.1, <a
     *            href="http://tools.ietf.org/html/rfc7208#section-14.1">The SPF
     *            DNS Record Type</a>.
     ...
     *            If IPv6 is enabled for the distribution, create two resource
     *            record sets to route traffic to your distribution, one with a
     *            value of <code>A</code> and one with a value of
     *            <code>AAAA</code>.
     *            </p>
     *            </li>
     *            <li>
     ...
     *            <li>
     *            <p>
     *            <b>Another resource record set in this hosted zone:</b>
     *            Specify the type of the resource record set that you're
     *            creating the alias for. All values are supported except
     *            <code>NS</code> and <code>SOA</code>.
     *            </p>
     *            <note>
     *            <p>
     *            If you're creating an alias record that has the same name as
     *            the hosted zone (known as the zone apex), you can't route
     *            traffic to a record for which the value of <code>Type</code>
     *            is <code>CNAME</code>. This is because the alias record must
     *            have the same type as the record you're routing traffic to,
     *            and creating a CNAME record for the zone apex isn't supported
     ...
     *         simple:</i> An identifier that differentiates among multiple
     *         resource record sets that have the same combination of name and
     *         type, such as multiple weighted resource record sets named
     *         acme.example.com that have a type of A. In a group of resource
     *         record sets that have the same name and type, the value of
     *         <code>SetIdentifier</code> must be unique for each resource
     *         record set.
     *         </p>
     *         <p>
     *         For information about routing policies, see <a href=
     ...
     *         53 responds to using the current resource record set. Route 53
     ...
     *         then responds to queries based on the ratio of a resource's
     ...
     *         record set, Route 53 never responds to queries with the
     ...
     *         "https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover-configuring-options.html"
     *         >Options for Configuring Route 53 Active-Active and
     ...
     *
     * @param weight <p>
     *            <i>Weighted resource record sets only:</i> Among resource
     *            record sets that have the same combination of DNS name and
     *            type, a value that determines the proportion of DNS queries
     *            that Amazon Route 53 responds to using the current resource
     *            record set. Route 53 calculates the sum of the weights for the
     *            resource record sets that have the same combination of DNS
     *            name and type. Route 53 then responds to queries based on the
     *            ratio of a resource's weight to the total. Note the following:
     *            </p>
     *            <ul>
     *            <li>
     *            <p>
     *            You must specify a value for the <code>Weight</code> element
     *            for every weighted resource record set.
     *            </p>
     *            </li>
     *            <li>
     *            <p>
     *            You can only specify one <code>ResourceRecord</code> per
     *            weighted resource record set.
     *            </p>
     *            </li>
     *            <li>
     ...
     *            <code>Name</code> and <code>Type</code> elements as weighted
     ...
     *            record set, Route 53 never responds to queries with the
     *            applicable value for that resource record set. However, if you
     *            set <code>Weight</code> to <code>0</code> for all resource
     *            record sets that have the same combination of DNS name and
     *            type, traffic is routed to all resources with equal
     *            probability.
     *            </p>
     *            <p>
     *            The effect of setting <code>Weight</code> to <code>0</code> is
     *            different when you associate health checks with weighted
     *            resource record sets. For more information, see <a href=
     *            "https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover-configuring-options.html"
     *            >Options for Configuring Route 53 Active-Active and
     *            Active-Passive Failover</a> in the <i>Amazon Route 53
     *            Developer Guide</i>.
     *            </p>
     *            </li>
     *            </ul>
     ...
     *         Region where you created the resource that this resource record
     *         set refers to. The resource typically is an AWS resource, such as
     *         an EC2 instance or an ELB load balancer, and is referred to by an
     *         IP address or a DNS domain name, depending on the record type.
     *         </p>
     *         <note>
     *         <p>
     *         Although creating latency and latency alias resource record sets
     *         in a private hosted zone is allowed, it's not supported.
     *         </p>
     *         </note>
     *         <p>
     ...
     *         Region. Route 53 then returns the value that is associated with
     ...
     * @param region <p>
     *            <i>Latency-based resource record sets only:</i> The Amazon EC2
     *            Region where you created the resource that this resource
     *            record set refers to. The resource typically is an AWS
     *            resource, such as an EC2 instance or an ELB load balancer, and
     *            is referred to by an IP address or a DNS domain name,
     *            depending on the record type.
     *            </p>
     *            <note>
     *            <p>
     *            Although creating latency and latency alias resource record
     ...
     *            Amazon EC2 Region. Route 53 then returns the value that is
     ...
     *            <p>
     *            You aren't required to create latency resource record sets for
     *            all Amazon EC2 Regions. Route 53 will choose the region with
     *            the best latency from among the regions that you create
     *            latency resource record sets for.
     *            </p>
     *            </li>
     *            <li>
     *            <p>
     *            You can't create non-latency resource record sets that have
     *            the same values for the <code>Name</code> and
     ...
     *
     * @return <p>
     *         <i>Geolocation resource record sets only:</i> A complex type that
     ...
     *         <code>ContinentCode</code> of <code>AF</code>.
     *         </p>
     *         <note>
     *         <p>
     *         Although creating geolocation and geolocation alias resource
     *         record sets in a private hosted zone is allowed, it's not
     *         supported.
     *         </p>
     *         </note>
     *         <p>
     ...
     *         </p>
     *         <p>
     *         You can't create two geolocation resource record sets that
     ...
     *         seven continents, Route 53 will receive some DNS queries from
     *         locations that it can't identify. We recommend that you create a
     *         resource record set for which the value of
     *         <code>CountryCode</code> is <code>*</code>. Two groups of queries
     ...
     *         <code>*</code> resource record set, Route 53 returns a
     *         "no answer" response for queries from those locations.
     *         </p>
     *         </important>
     *         <p>
     *         You can't create non-geolocation resource record sets that have
     ...
     *
     * @param geoLocation <p>
     *            <i>Geolocation resource record sets only:</i> A complex type
     ...
     *            If you create separate resource record sets for overlapping
     *            geographic regions (for example, one resource record set for a
     *            continent and one for a country on the same continent),
     *            priority goes to the smallest geographic region. This allows
     *            you to route most queries for a continent to one resource and
     *            to route queries for a country on that continent to a
     *            different resource.
     *            </p>
     *            <p>
     *            You can't create two geolocation resource record sets that
     ...
     *            </p>
     *            <p>
     *            The value <code>*</code> in the <code>CountryCode</code>
     ...
     *            values for the <code>Name</code> and <code>Type</code>
     *            elements.
     *            </p>
     *            <important>
     *            <p>
     *            Geolocation works by mapping IP addresses to locations.
     *            However, some IP addresses aren't mapped to geographic
     *            locations, so even if you create geolocation resource record
     *            sets that cover all seven continents, Route 53 will receive
     *            some DNS queries from locations that it can't identify. We
     *            recommend that you create a resource record set for which the
     *            value of <code>CountryCode</code> is <code>*</code>. Two
     *            groups of queries are routed to the resource that you specify
     *            in this record: queries that come from locations for which you
     *            haven't created geolocation resource record sets and queries
     *            from IP addresses that aren't mapped to a location. If you
     *            don't create a <code>*</code> resource record set, Route 53
     ...
     *         <li>
     *         <p>
     *         When the primary resource record set is healthy, Route 53
     ...
     *         set is unhealthy, Route 53 always responds to DNS queries with
     *         the applicable value from the secondary resource record set. This
     ...
     *         the following topics in the <i>Amazon Route 53 Developer
     *         Guide</i>:
     *         </p>
     *         <ul>
     *         <li>
     ...
     *
     * @param failover <p>
     *            <i>Failover resource record sets only:</i> To configure
     *            failover, you add the <code>Failover</code> element to two
     *            resource record sets. For one resource record set, you specify
     *            <code>PRIMARY</code> as the value for <code>Failover</code>;
     *            for the other resource record set, you specify
     *            <code>SECONDARY</code>. In addition, you include the
     *            <code>HealthCheckId</code> element and specify the health
     *            check that you want Amazon Route 53 to perform for each
     *            resource record set.
     *            </p>
     *            <p>
     *            Except where noted, the following failover behaviors assume
     *            that you have included the <code>HealthCheckId</code> element
     ...
     *            <p>
     *            If you omit the <code>HealthCheckId</code> element for the
     *            secondary resource record set, and if the primary resource
     *            record set is unhealthy, Route 53 always responds to DNS
     *            queries with the applicable value from the secondary resource
     *            record set. This is true regardless of the health of the
     *            associated endpoint.
     *            </p>
     *            </li>
     *            </ul>
     *            <p>
     *            You can't create non-failover resource record sets that have
     *            the same values for the <code>Name</code> and
     *            <code>Type</code> elements as failover resource record sets.
     *            </p>
     *            <p>
     *            For failover alias resource record sets, you must also include
     *            the <code>EvaluateTargetHealth</code> element and set the
     *            value to true.
     *            </p>
     *            <p>
     *            For more information about configuring failover for Route 53,
     *            see the following topics in the <i>Amazon Route 53 Developer
     *            Guide</i>:
     *            </p>
     *            <ul>
     *            <li>
     ...
     *
     * @return <p>
     *         <i>Multivalue answer resource record sets only</i>: To route
     *         traffic approximately randomly to multiple resources, such as web
     *         servers, create one multivalue answer record for each resource
     *         and specify <code>true</code> for <code>MultiValueAnswer</code>.
     *         Note the following:
     *         </p>
     *         <ul>
     *         <li>
     ...
     *         <li>
     *         <p>
     *         Route 53 responds to DNS queries with up to eight healthy
     *         records; if you have eight or fewer healthy records, Route 53
     ...
     *            <code>MultiValueAnswer</code>. Note the following:
     *            </p>
     *            <ul>
     *            <li>
     *            <p>
     ...
     *            <li>
     *            <p>
     *            If you have more than eight healthy records, Route 53 responds
     *            to different DNS resolvers with different combinations of
     *            healthy records.
     *            </p>
     *            </li>
     *            <li>
     ...
     *         record sets must have the same value for <code>TTL</code>.
     *         </p>
     *         </li>
     *         <li>
     *         <p>
     ...
     *            <li>
     *            <p>
     *            If you're creating or updating an alias resource record set,
     *            omit <code>TTL</code>. Amazon Route 53 uses the value of
     *            <code>TTL</code> for the alias target.
     *            </p>
     *            </li>
     *            <li>
     *            <p>
     *            If you're associating this resource record set with a health
     *            check (if you're adding a <code>HealthCheckId</code> element),
     *            we recommend that you specify a <code>TTL</code> of 60 seconds
     *            or less so clients respond quickly to changes in health
     *            status.
     *            </p>
     *            </li>
     *            <li>
     ...
     *            target is an ELB load balancer, we recommend that you specify
     *            a <code>TTL</code> of 60 seconds for all of the non-alias
     *            weighted resource record sets that have the same name and
     *            type. Values other than 60 seconds (the TTL for load
     *            balancers) will change the effect of the values that you
     *            specify for <code>Weight</code>.
     *            </p>
     *            </li>
     *            </ul>
     */
    ...
    public void setResourceRecords(java.util.Collection<ResourceRecord> resourceRecords) {
        ...
        this.resourceRecords = new java.util.ArrayList<ResourceRecord>(resourceRecords);
    }
    ...
    public ResourceRecordSet withResourceRecords(ResourceRecord... resourceRecords) {
        if (getResourceRecords() == null) {
            this.resourceRecords = new java.util.ArrayList<ResourceRecord>(resourceRecords.length);
        }
        for (ResourceRecord value : resourceRecords) {
            this.resourceRecords.add(value);
        }
        return this;
    }
     ...
     *            AWS resource, such as a CloudFront distribution or an Amazon
     ...
     *            alias resource record sets in a private hosted zone is
     *            unsupported.
     *            </p>
     *            </li>
     *            <li>
     *            <p>
     *            For information about creating failover resource record sets
     *            in a private hosted zone, see <a href=
     *            "https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover-private-hosted-zones.html"
     *            >Configuring Failover in a Private Hosted Zone</a> in the
     *            <i>Amazon Route 53 Developer Guide</i>.
     *            </p>
     ...
     *         If you want Amazon Route 53 to return this resource record set in
     *         response to a DNS query only when the status of a health check is
     *         healthy, include the <code>HealthCheckId</code> element and
     *         specify the ID of the applicable health check.
     *         </p>
     *         <p>
     *         Route 53 determines whether a resource record set is healthy
     ...
     *         <important>
     *         <p>
     *         Route 53 doesn't check the health of the endpoint that is
     ...
     *         </important>
     *         <p>
     *         For more information, see the following topics in the <i>Amazon
     *         Route 53 Developer Guide</i>:
     *         </p>
     *         <ul>
     *         <li>
     ...
     *         </p>
     *         <p>
     *         Specifying a value for <code>HealthCheckId</code> is useful only
     *         when Route 53 is choosing between two or more resource record
     *         sets to respond to a DNS query, and you want Route 53 to base the
     ...
     *         <li>
     *         <p>
     *         <b>Non-alias resource record sets</b>: You're checking the health
     ...
     *         Route 53 includes the record among the records that it responds
     *         to DNS queries with.
     *         </p>
     *         <p>
     *         If the health check status for a resource record set is
     *         unhealthy, Route 53 stops responding to DNS queries using the
     ...
     *         <li>
     *         <p>
     *         <b>Alias resource record sets</b>: You specify the following
     *         settings:
     *         </p>
     *         <ul>
     *         <li>
     *         <p>
     *         You set <code>EvaluateTargetHealth</code> to true for an alias
     *         resource record set in a group of resource record sets that have
     ...
     *         </p>
     *         <p>
     *         If the health check status is unhealthy, Route 53 stops
     *         responding to DNS queries using the alias resource record set.
     *         </p>
     *         <note>
     *         <p>
     ...
     *         health checks with all of the resource record sets in the group
     ...
     *         unhealthy, Route 53 looks for a resource record set for the
     ...
     *         the entire United States, for North America, and a resource
     *         record set that has <code>*</code> for <code>CountryCode</code>
     *         is <code>*</code>, which applies to all locations. If the
     *         endpoint for the state resource record set is unhealthy, Route 53
     *         checks for healthy resource record sets in the following order
     *         until it finds a resource record set for which the endpoint is
     *         healthy:
     *         </p>
     *         <ul>
     *         <li>
     ...
     *         the name of the resource record sets (
     *         <code>www.example.com</code>).
     *         </p>
     *         <important>
     *         <p>
     *         Health check results will be unpredictable if you do the
     *         following:
     *         </p>
     *         <ul>
     *         <li>
     ...
     *            in response to a DNS query only when the status of a health
     *            check is healthy, include the <code>HealthCheckId</code>
     ...
     *            <p>
     *            Route 53 doesn't check the health of the endpoint that is
     *            specified in the resource record set, for example, the
     *            endpoint specified by the IP address in the <code>Value</code>
     *            element. When you add a <code>HealthCheckId</code> element to
     *            a resource record set, Route 53 checks the health of the
     ...
     *            </p>
     *            <p>
     *            Specifying a value for <code>HealthCheckId</code> is useful
     *            only when Route 53 is choosing between two or more resource
     *            record sets to respond to a DNS query, and you want Route 53
     *            to base the choice in part on the status of a health check.
     ...
     *            the same routing policy, name, and type (such as multiple
     *            weighted records named www.example.com with a type of A) and
     *            you specify health check IDs for all the resource record sets.
     *            </p>
     *            <p>
     *            If the health check status for a resource record set is
     *            healthy, Route 53 includes the record among the records that
     *            it responds to DNS queries with.
     *            </p>
     *            <p>
     *            If the health check status for a resource record set is
     *            unhealthy, Route 53 stops responding to DNS queries using the
     ...
     *            <p>
     *            If the health check status for all resource record sets in the
     *            group is unhealthy, Route 53 considers all resource record
     *            sets in the group healthy and responds to DNS queries
     *            accordingly.
     *            </p>
     *            </li>
     *            <li>
     ...
     *            <p>
     *            You set <code>EvaluateTargetHealth</code> to true for an alias
     *            resource record set in a group of resource record sets that
     *            have the same routing policy, name, and type (such as multiple
     *            weighted records named www.example.com with a type of A).
     *            </p>
     *            </li>
     *            <li>
     *            <p>
     *            You configure the alias resource record set to route traffic
     *            to a non-alias resource record set in the same hosted zone.
     *            </p>
     *            </li>
     *            <li>
     *            <p>
     ...
     *            <p>
     *            The alias resource record set can also route traffic to a
     *            <i>group</i> of non-alias resource record sets that have the
     ...
     *            unhealthy, Route 53 looks for a resource record set for the
     *            larger, associated geographic region. For example, suppose you
     *            have resource record sets for a state in the United States,
     *            for the entire United States, for North America, and a
     *            resource record set that has <code>*</code> for
     *            <code>CountryCode</code> is <code>*</code>, which applies to
     *            all locations. If the endpoint for the state resource record
     *            set is unhealthy, Route 53 checks for healthy resource record
     *            sets in the following order until it finds a resource record
     ...
     *            </p>
     *            <p>
     *            If your health checks specify the endpoint only by domain
     *            name, we recommend that you create a separate health check for
     *            each endpoint. For example, create a health check for each
     *            <code>HTTP</code> server that is serving content for
     *            <code>www.example.com</code>. For the value of
     ...
     *            <p>
     *            Create a health check that has the same value for
     *            <code>FullyQualifiedDomainName</code> as the name of a
     *            resource record set.
     *            </p>
     *            </li>
     *            <li>
     ...
     *         the resource record set automatically. If you delete the resource
     ...
     * <b>Length: </b>1 - 36<br/>
     *
     * @param trafficPolicyInstanceId <p>
     *            When you create a traffic policy instance, Amazon Route 53
     *            automatically creates a resource record set.
     *            <code>TrafficPolicyInstanceId</code> is the ID of the traffic
     *            policy instance that Route 53 created this resource record set
    ...
    public String toString() {
        ...
        if (getMultiValueAnswer() != null)
            sb.append("MultiValueAnswer: " + getMultiValueAnswer() + ",");
        if (getTTL() != null)
    ...
    public int hashCode() {
        ...
        hashCode = prime * hashCode
                + ((getSetIdentifier() == null) ? 0 : getSetIdentifier().hashCode());
        ...
        hashCode = prime * hashCode
                + ((getMultiValueAnswer() == null) ? 0 : getMultiValueAnswer().hashCode());
        hashCode = prime * hashCode + ((getTTL() == null) ? 0 : getTTL().hashCode());
        hashCode = prime * hashCode
                + ((getResourceRecords() == null) ? 0 : getResourceRecords().hashCode());
        hashCode = prime * hashCode
        ...
        hashCode = prime
                * hashCode
                + ((getTrafficPolicyInstanceId() == null) ? 0 : getTrafficPolicyInstanceId()
                        .hashCode());
        return hashCode;
    }
