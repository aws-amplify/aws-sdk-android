...
public abstract class PaginatedList<T> implements List<T> {
    ...
    /**
     * All currently loaded results for this list. In ITERATION_ONLY mode, this
     * list will at most keep one page of the loaded results, and all previous
     * results will be cleared from the memory.
     */
    ...
    /**
     * Keeps track on whether an iterator of the list has been retrieved. Only
     * updated and checked when the list is in ITERATION_ONLY mode.
     */
     ...
     * @param dynamo The DynamoDB client for making low-level request calls.
     * @param paginationLoadingStrategy The strategy used for loading paginated
    ...
    public PaginatedList(DynamoDBMapper mapper, Class<T> clazz, AmazonDynamoDB dynamo,
        ...
        // Ideally, we should eagerly load all results here as soon as
        // EAGER_LOADING is configured.
        // But the implementation of loadAllResults() relies on a fully
        // initialized sub-class object.
     ...
     * necessary.
     * <p>
     * If it configured with ITERARTION_ONLY mode, then the iterator could be
     * only retrieved once, and any previously loaded results will be cleared in
     * the memory during the iteration.
     * </p>
     */
    @Override
    public Iterator<T> iterator() {
        return new PaginatedListIterator(
                paginationLoadingStrategy == PaginationLoadingStrategy.ITERATION_ONLY);
    }
    ...
    private class PaginatedListIterator implements Iterator<T> {
         ...
         * ConcurrentModificationExceptions. Only needed when the list is not in
         * ITERNATION_ONLY mode.
         */
        ...
        public PaginatedListIterator(boolean iterationOnly) {
            ...
            if (iterationOnly) {
                synchronized (PaginatedList.this) {
                    if (iterationStarted) {
                        throw new UnsupportedOperationException(
                                "The list could only be iterated once in ITERATION_ONLY mode.");
                    }
                ...
                /*
                 * We make a copy of the allResults list to iterate over in
                 * order to avoid ConcurrentModificationExceptions caused by
                 * other methods loading more results into the list while
                 * someone iterates over it. This is a more severe problem than
                 * it might seem, because even innocuous-seeming operations such
                 * as contains() can modify the underlying result set.
                 */
        ...
        public T next() {
            if (!innerIterator.hasNext()) {
                 ...
                 * if -- it's in ITERATION_ONLY mode (which means innerIterator
                 * is always pointing at the "real" list of loaded results) OR
                 * it's not in ITERATION_ONLY and our private copy of the result
                 * list is already up to date with the full one.
                 */
                if (iterationOnly
                ...
                if (iterationOnly) {
                    /*
                     * allResults has been replaced with the latest page of
                     * results.
                     */
                    innerIterator = allResults.iterator();
                } else {
    ...
    void checkUnsupportedOperationForIterationOnlyMode(String methodSignature) {
