 ...
 * </p>
 * <p>
 * A single <code>Scan</code> operation reads up to the maximum number of items
 * set (if using the <code>Limit</code> parameter) or a maximum of 1 MB of data
 * and then apply any filtering to the results using
 * <code>FilterExpression</code>. If <code>LastEvaluatedKey</code> is present in
 * the response, you need to paginate the result set. For more information, see
 * <a href=
...
public class ScanRequest extends AmazonWebServiceRequest implements Serializable {
     ...
     * that are projected into that index, the operation reads only the index
     ...
     * >Specifying Item Attributes</a> in the <i>Amazon DynamoDB Developer
     * Guide</i>.
     * </p>
     */
     ...
     * the value of the <code>ProductStatus</code> attribute was one of the
     * following:
     * </p>
     * <p>
     ...
     *         the processed dataset size exceeds 1 MB before DynamoDB reaches
     ...
     *         "https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/QueryAndScan.html"
     *         >Working with Queries</a> in the <i>Amazon DynamoDB Developer
     *         Guide</i>.
     *         </p>
     */
     ...
     *            off. Also, if the processed dataset size exceeds 1 MB before
     ...
     *         index, then for each matching item in the index, DynamoDB fetches
     ...
     *         reads only the index and not the table. If any of the requested
     ...
     *         This extra fetching incurs additional throughput cost and
     *         latency.
     *         </p>
     *         <p>
     ...
     *            index, DynamoDB fetches the entire item from the parent table.
     *            If the index is configured to project all item attributes,
     *            then all of the data can be obtained from the local secondary
     *            index, and no fetching is required.
     *            </p>
     *            </li>
     *            <li>
     ...
     *            If you query or scan a local secondary index and request only
     *            attributes that are projected into that index, the operation
     *            reads only the index and not the table. If any of the
     *            requested attributes are not projected into the local
     *            secondary index, DynamoDB fetches each of these attributes
     ...
     *         For more information, see <a href=
     *         "https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.AccessingItemAttributes.html"
     *         >Specifying Item Attributes</a> in the <i>Amazon DynamoDB
     *         Developer Guide</i>.
     *         </p>
     */
     ...
     *         wanted to check whether the value of the
     *         <code>ProductStatus</code> attribute was one of the following:
     *         </p>
     *         <p>
     *         <code>Available | Backordered | Discontinued</code>
     *         </p>
     ...
     *         For more information on expression attribute values, see <a href=
     *         "https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.SpecifyingConditions.html"
     *         >Condition Expressions</a> in the <i>Amazon DynamoDB Developer
     *         Guide</i>.
     *         </p>
     */
