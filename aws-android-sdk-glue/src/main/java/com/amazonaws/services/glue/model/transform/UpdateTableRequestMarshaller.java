...
public class UpdateTableRequestMarshaller implements
    ...
    public Request<UpdateTableRequest> marshall(UpdateTableRequest updateTableRequest) {
        ...
        try {
            ...
            if (updateTableRequest.getCatalogId() != null) {
                String catalogId = updateTableRequest.getCatalogId();
                jsonWriter.name("CatalogId");
                jsonWriter.value(catalogId);
            }
            if (updateTableRequest.getDatabaseName() != null) {
                String databaseName = updateTableRequest.getDatabaseName();
                jsonWriter.name("DatabaseName");
                jsonWriter.value(databaseName);
            }
            if (updateTableRequest.getTableInput() != null) {
                TableInput tableInput = updateTableRequest.getTableInput();
                jsonWriter.name("TableInput");
                TableInputJsonMarshaller.getInstance().marshall(tableInput, jsonWriter);
            }
            if (updateTableRequest.getSkipArchive() != null) {
                Boolean skipArchive = updateTableRequest.getSkipArchive();
                jsonWriter.name("SkipArchive");
                jsonWriter.value(skipArchive);
            }
