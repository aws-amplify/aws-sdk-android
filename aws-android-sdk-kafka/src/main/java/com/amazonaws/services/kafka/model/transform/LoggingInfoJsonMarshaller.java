...
class LoggingInfoJsonMarshaller {
    ...
    public void marshall(LoggingInfo loggingInfo, AwsJsonWriter jsonWriter) throws Exception {
        ...
        if (loggingInfo.getBrokerLogs() != null) {
            BrokerLogs brokerLogs = loggingInfo.getBrokerLogs();
            jsonWriter.name("BrokerLogs");
            BrokerLogsJsonMarshaller.getInstance().marshall(brokerLogs, jsonWriter);
        }
