...
class CaptionSelectorJsonMarshaller {
    ...
    public void marshall(CaptionSelector captionSelector, AwsJsonWriter jsonWriter)
        ...
        if (captionSelector.getName() != null) {
            String name = captionSelector.getName();
            jsonWriter.name("Name");
            jsonWriter.value(name);
        }
        if (captionSelector.getSelectorSettings() != null) {
            CaptionSelectorSettings selectorSettings = captionSelector.getSelectorSettings();
            jsonWriter.name("SelectorSettings");
            CaptionSelectorSettingsJsonMarshaller.getInstance().marshall(selectorSettings,
                    jsonWriter);
        }
