 ...
 * Amazon Virtual Private Cloud configurations. An AWS account is limited to one
 * Domain, per region. Users within a domain can share notebook files and other
 * artifacts with each other. When a Domain is created, an Amazon Elastic File
 * System (EFS) is also created for use by all of the users within the Domain.
 * Each user receives a private home directory within the EFS for notebooks, Git
