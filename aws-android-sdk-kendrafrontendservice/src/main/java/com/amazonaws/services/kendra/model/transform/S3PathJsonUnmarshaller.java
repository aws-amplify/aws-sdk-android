...
/**
 * JSON unmarshaller for POJO S3Path
 */
class S3PathJsonUnmarshaller implements Unmarshaller<S3Path, JsonUnmarshallerContext> {
    ...
    public S3Path unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (reader.hasNext()) {
            ...
            if (name.equals("Bucket")) {
                s3Path.setBucket(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("Key")) {
                s3Path.setKey(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else {
                reader.skipValue();
            }
