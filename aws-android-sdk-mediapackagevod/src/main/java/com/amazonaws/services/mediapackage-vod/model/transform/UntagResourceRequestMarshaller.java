...
public class UntagResourceRequestMarshaller implements Marshaller<Request<UntagResourceRequest>, UntagResourceRequest> {
    ...
    public Request<UntagResourceRequest> marshall(UntagResourceRequest untagResourceRequest) {
        ...
        Request<UntagResourceRequest> request = new DefaultRequest<UntagResourceRequest>(untagResourceRequest, "AWSElementalMediaPackageVOD");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/tags/{resource-arn}";
        uriResourcePath = uriResourcePath.replace("{resource-arn}", (untagResourceRequest.getResourceArn() == null) ? "" : StringUtils.fromString(untagResourceRequest.getResourceArn()));
        if (untagResourceRequest.getTagKeys() != null) {
            request.addParameter("tagKeys", StringUtils.join(",", untagResourceRequest.getTagKeys().toArray(new String[0])));
        }
