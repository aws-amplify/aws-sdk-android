...
class ModerationLabelJsonMarshaller {
    ...
    public void marshall(ModerationLabel moderationLabel, AwsJsonWriter jsonWriter)
        ...
        if (moderationLabel.getTaxonomyLevel() != null) {
            Integer taxonomyLevel = moderationLabel.getTaxonomyLevel();
            jsonWriter.name("TaxonomyLevel");
            jsonWriter.value(taxonomyLevel);
        }
