...
public class PredictionJsonUnmarshaller implements Unmarshaller<Prediction, JsonUnmarshallerContext> {
    ...
    public Prediction unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (reader.hasNext()) {
            ...
            if (name.equals("predictedValue")) {
                prediction.setPredictedValue(FloatJsonUnmarshaller.getInstance().unmarshall(context));
            } else 
            if (name.equals("predictedScores")) {
                prediction.setPredictedScores(new MapUnmarshaller<Float>(FloatJsonUnmarshaller.getInstance()).unmarshall(context));
            } else 
            if (name.equals("details")) {
                prediction.setDetails(new MapUnmarshaller<String>(StringJsonUnmarshaller.getInstance()).unmarshall(context));
            } else 
            {
                reader.skipValue();
            }
