...
/**
 * JSON request marshaller for ListRepositoriesRequest
 */
    ...
    public Request<ListRepositoriesRequest> marshall(ListRepositoriesRequest listRepositoriesRequest) {
        if (listRepositoriesRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(ListRepositoriesRequest)");
        }

        Request<ListRepositoriesRequest> request = new DefaultRequest<ListRepositoriesRequest>(
                listRepositoriesRequest, "CodeArtifact");
        request.setHttpMethod(HttpMethodName.POST);

        String uriResourcePath = "/v1/repositories";
        if (listRepositoriesRequest.getRepositoryPrefix() != null) {
            request.addParameter("repository-prefix",
                    StringUtils.fromString(listRepositoriesRequest.getRepositoryPrefix()));
        }
        if (listRepositoriesRequest.getMaxResults() != null) {
            request.addParameter("max-results",
                    StringUtils.fromInteger(listRepositoriesRequest.getMaxResults()));
        }
        if (listRepositoriesRequest.getNextToken() != null) {
            request.addParameter("next-token",
                    StringUtils.fromString(listRepositoriesRequest.getNextToken()));
        }
