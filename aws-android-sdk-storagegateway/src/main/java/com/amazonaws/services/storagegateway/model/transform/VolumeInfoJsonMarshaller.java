/*
 * Copyright 2010-2020 Amazon.com, Inc. or its affiliates. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License").
 * You may not use this file except in compliance with the License.
 * A copy of the License is located at
 *
 *  http://aws.amazon.com/apache2.0
 *
 * or in the "license" file accompanying this file. This file is distributed
 * on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */

package com.amazonaws.services.storagegateway.model.transform;

import com.amazonaws.services.storagegateway.model.*;
import com.amazonaws.util.json.AwsJsonWriter;

/**
 * JSON marshaller for POJO VolumeInfo
 */
class VolumeInfoJsonMarshaller {

    public void marshall(VolumeInfo volumeInfo, AwsJsonWriter jsonWriter) throws Exception {
        jsonWriter.beginObject();
        if (volumeInfo.getVolumeARN() != null) {
            String volumeARN = volumeInfo.getVolumeARN();
            jsonWriter.name("VolumeARN");
            jsonWriter.value(volumeARN);
        }
        if (volumeInfo.getVolumeId() != null) {
            String volumeId = volumeInfo.getVolumeId();
            jsonWriter.name("VolumeId");
            jsonWriter.value(volumeId);
        }
        if (volumeInfo.getGatewayARN() != null) {
            String gatewayARN = volumeInfo.getGatewayARN();
            jsonWriter.name("GatewayARN");
            jsonWriter.value(gatewayARN);
        }
        if (volumeInfo.getGatewayId() != null) {
            String gatewayId = volumeInfo.getGatewayId();
            jsonWriter.name("GatewayId");
            jsonWriter.value(gatewayId);
        }
        if (volumeInfo.getVolumeType() != null) {
            String volumeType = volumeInfo.getVolumeType();
            jsonWriter.name("VolumeType");
            jsonWriter.value(volumeType);
        }
        if (volumeInfo.getVolumeSizeInBytes() != null) {
            Long volumeSizeInBytes = volumeInfo.getVolumeSizeInBytes();
            jsonWriter.name("VolumeSizeInBytes");
            jsonWriter.value(volumeSizeInBytes);
        }
        if (volumeInfo.getVolumeAttachmentStatus() != null) {
            String volumeAttachmentStatus = volumeInfo.getVolumeAttachmentStatus();
            jsonWriter.name("VolumeAttachmentStatus");
            jsonWriter.value(volumeAttachmentStatus);
        }
        jsonWriter.endObject();
    }

    private static VolumeInfoJsonMarshaller instance;

    public static VolumeInfoJsonMarshaller getInstance() {
        if (instance == null)
            instance = new VolumeInfoJsonMarshaller();
        return instance;
    }
}
