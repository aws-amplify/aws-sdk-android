...
/**
 * JSON request marshaller for DescribeStreamRequest
 */
    ...
    public Request<DescribeStreamRequest> marshall(DescribeStreamRequest describeStreamRequest) {
        if (describeStreamRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DescribeStreamRequest)");
        }

        Request<DescribeStreamRequest> request = new DefaultRequest<DescribeStreamRequest>(
                describeStreamRequest, "AWSIot");
        request.setHttpMethod(HttpMethodName.GET);

        String uriResourcePath = "/streams/{streamId}";
        uriResourcePath = uriResourcePath.replace(
                "{streamId}",
                (describeStreamRequest.getStreamId() == null) ? "" : StringUtils
                        .fromString(describeStreamRequest.getStreamId()));
