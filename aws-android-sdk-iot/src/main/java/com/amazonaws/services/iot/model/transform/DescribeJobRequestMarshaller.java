...
/**
 * JSON request marshaller for DescribeJobRequest
 */
    ...
    public Request<DescribeJobRequest> marshall(DescribeJobRequest describeJobRequest) {
        if (describeJobRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DescribeJobRequest)");
        }

        Request<DescribeJobRequest> request = new DefaultRequest<DescribeJobRequest>(
                describeJobRequest, "AWSIot");
        request.setHttpMethod(HttpMethodName.GET);

        String uriResourcePath = "/jobs/{jobId}";
        uriResourcePath = uriResourcePath.replace(
                "{jobId}",
                (describeJobRequest.getJobId() == null) ? "" : StringUtils
                        .fromString(describeJobRequest.getJobId()));
