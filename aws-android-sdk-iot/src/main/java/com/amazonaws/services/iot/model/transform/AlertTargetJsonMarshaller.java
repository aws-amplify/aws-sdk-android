...
/**
 * JSON marshaller for POJO AlertTarget
 */
class AlertTargetJsonMarshaller {

    public void marshall(AlertTarget alertTarget, AwsJsonWriter jsonWriter) throws Exception {
        jsonWriter.beginObject();
        if (alertTarget.getAlertTargetArn() != null) {
            String alertTargetArn = alertTarget.getAlertTargetArn();
            jsonWriter.name("alertTargetArn");
            jsonWriter.value(alertTargetArn);
        }
