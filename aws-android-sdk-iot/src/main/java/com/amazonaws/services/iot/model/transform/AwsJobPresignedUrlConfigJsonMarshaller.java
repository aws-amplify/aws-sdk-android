...
/**
 * JSON marshaller for POJO AwsJobPresignedUrlConfig
 */
class AwsJobPresignedUrlConfigJsonMarshaller {

    public void marshall(AwsJobPresignedUrlConfig awsJobPresignedUrlConfig, AwsJsonWriter jsonWriter)
            throws Exception {
        jsonWriter.beginObject();
        if (awsJobPresignedUrlConfig.getExpiresInSec() != null) {
            Long expiresInSec = awsJobPresignedUrlConfig.getExpiresInSec();
            jsonWriter.name("expiresInSec");
            jsonWriter.value(expiresInSec);
        }
