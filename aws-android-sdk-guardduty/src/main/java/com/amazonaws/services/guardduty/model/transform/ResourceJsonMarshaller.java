...
class ResourceJsonMarshaller {
    ...
    public void marshall(Resource resource, AwsJsonWriter jsonWriter) throws Exception {
        ...
        if (resource.getS3BucketDetails() != null) {
            java.util.List<S3BucketDetail> s3BucketDetails = resource.getS3BucketDetails();
            jsonWriter.name("S3BucketDetails");
            jsonWriter.beginArray();
            for (S3BucketDetail s3BucketDetailsItem : s3BucketDetails) {
                if (s3BucketDetailsItem != null) {
                    S3BucketDetailJsonMarshaller.getInstance().marshall(s3BucketDetailsItem,
                            jsonWriter);
                }
            }
            jsonWriter.endArray();
