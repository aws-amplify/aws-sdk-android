...
/**
 * JSON request marshaller for DeleteIPSetRequest
 */
    ...
    public Request<DeleteIPSetRequest> marshall(DeleteIPSetRequest deleteIPSetRequest) {
        if (deleteIPSetRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeleteIPSetRequest)");
        }

        Request<DeleteIPSetRequest> request = new DefaultRequest<DeleteIPSetRequest>(
                deleteIPSetRequest, "AmazonGuardDuty");
        request.setHttpMethod(HttpMethodName.DELETE);

        String uriResourcePath = "/detector/{detectorId}/ipset/{ipSetId}";
        uriResourcePath = uriResourcePath.replace(
                "{detectorId}",
                (deleteIPSetRequest.getDetectorId() == null) ? "" : StringUtils
                        .fromString(deleteIPSetRequest.getDetectorId()));
        uriResourcePath = uriResourcePath.replace(
                "{ipSetId}",
                (deleteIPSetRequest.getIpSetId() == null) ? "" : StringUtils
                        .fromString(deleteIPSetRequest.getIpSetId()));
