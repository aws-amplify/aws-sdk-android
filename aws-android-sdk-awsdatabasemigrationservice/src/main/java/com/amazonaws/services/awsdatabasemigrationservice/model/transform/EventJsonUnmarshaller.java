    ...
    public Event unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (reader.hasNext()) {
            ...
            if (name.equals("SourceIdentifier")) {
                event.setSourceIdentifier(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("SourceType")) {
                event.setSourceType(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("Message")) {
                event.setMessage(StringJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else if (name.equals("EventCategories")) {
                event.setEventCategories(new ListUnmarshaller<String>(StringJsonUnmarshaller
                        .getInstance()
                        )
                                .unmarshall(context));
            } else if (name.equals("Date")) {
                event.setDate(DateJsonUnmarshaller.getInstance()
                        .unmarshall(context));
            } else {
                reader.skipValue();
            }
