 ...
 * >custom key store</a>. This operation does not affect any backing elements of
 * the custom key store. It does not delete the CloudHSM cluster that is
 * associated with an CloudHSM key store, or affect any users or keys in the
 * cluster. For an external key store, it does not affect the external key store
 * proxy, external key manager, or any external keys.
 * </p>
 * <p>
 ...
 * the KMS keys from the key store. After the required waiting period expires
 * and all KMS keys are deleted from the custom key store, use
 * <a>DisconnectCustomKeyStore</a> to disconnect the key store from KMS. Then,
 * you can delete the custom key store.
 * </p>
 * <p>
 * For keys in an CloudHSM key store, the <code>ScheduleKeyDeletion</code>
 * operation makes a best effort to delete the key material from the associated
 ...
 * >delete the orphaned key material</a> from the cluster and its backups. KMS
 * never creates, manages, or deletes cryptographic keys in the external key
 ...
 * from its backing key store. While the key store is disconnected, you cannot
