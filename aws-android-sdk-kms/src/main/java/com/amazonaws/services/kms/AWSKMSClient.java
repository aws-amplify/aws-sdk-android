...
public class AWSKMSClient extends AmazonWebServiceClient implements AWSKMS {
     ...
     * and verifying MACs) and the algorithms that the KMS key supports. For <a
     * href="kms/latest/developerguide/multi-region-keys-overview.html">multi-
     * Region keys</a>, it displays the primary key and all related replica
     * keys. For KMS keys in <a
     * href="kms/latest/developerguide/keystore-cloudhsm.html">CloudHSM key
     * stores</a>, it includes information about the custom key store, such as
     ...
     * stores</a>, it includes the custom key store ID and the ID and status of
     * the associated external key.
     * </p>
     * <p>
     ...
     * which you will import key material. The KMS key <code>Origin</code> must
     * be <code>EXTERNAL</code>. You must also specify the wrapping algorithm
     * and type of wrapping key (public key) that you will use to encrypt the
     * key material. You cannot perform this operation on an asymmetric KMS key,
