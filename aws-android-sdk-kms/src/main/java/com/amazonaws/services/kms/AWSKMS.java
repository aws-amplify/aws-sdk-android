...
public interface AWSKMS {
     ...
     * operation. This operation requires an asymmetric CMK with a
     * <code>KeyUsage</code> value of <code>SIGN_VERIFY</code>.
     * </p>
     * <p/>
     * <p>
     ...
     * in an asymmetric CMK. The signature is verified by using the public key
     * in the same asymmetric CMK. For information about symmetric and
     * asymmetric CMKs, see <a href=
     * "https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html"
     * >Using Symmetric and Asymmetric CMKs</a> in the <i>AWS Key Management
     * Service Developer Guide</i>.
     * </p>
     ...
     * operation. Specify the same asymmetric CMK that was used by the
     * <code>Sign</code> operation to generate the digital signature.
     * </p>
     * <p>
     ...
     * performed within AWS KMS. As a result, it's easy to call, the operation
     * is performed within the FIPS boundary, it is logged in AWS CloudTrail,
     * and you can use key policy and IAM policy to determine who is authorized
     * to use the CMK to verify signatures.
     * </p>
     * <important>
     * <p>
     * The result of the <code>Verify</code> operation, which is represented by
     * its HTTP status code, does not indicate whether the signature
     * verification succeeded or failed. To determine whether the signature was
     * verified, see the <code>SignatureValid</code> field in the response.
     * </p>
     * </important>
     * <p>
     * The CMK that you use for this operation must be in a compatible key
