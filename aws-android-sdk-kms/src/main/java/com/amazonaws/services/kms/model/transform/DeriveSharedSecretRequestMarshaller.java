...
/**
 * JSON request marshaller for DeriveSharedSecretRequest
 */
public class DeriveSharedSecretRequestMarshaller implements
        Marshaller<Request<DeriveSharedSecretRequest>, DeriveSharedSecretRequest> {

    public Request<DeriveSharedSecretRequest> marshall(
            DeriveSharedSecretRequest deriveSharedSecretRequest) {
        if (deriveSharedSecretRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DeriveSharedSecretRequest)");
        }

        Request<DeriveSharedSecretRequest> request = new DefaultRequest<DeriveSharedSecretRequest>(
                deriveSharedSecretRequest, "AWSKMS");
        ...
        try {
            ...
            if (deriveSharedSecretRequest.getPublicKey() != null) {
                java.nio.ByteBuffer publicKey = deriveSharedSecretRequest.getPublicKey();
                jsonWriter.name("PublicKey");
                jsonWriter.value(publicKey);
            }
            if (deriveSharedSecretRequest.getGrantTokens() != null) {
                java.util.List<String> grantTokens = deriveSharedSecretRequest.getGrantTokens();
