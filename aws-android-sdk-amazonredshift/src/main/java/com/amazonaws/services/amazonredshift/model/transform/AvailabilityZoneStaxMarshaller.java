...
class AvailabilityZoneStaxMarshaller {
    ...
    public void marshall(AvailabilityZone _availabilityZone, Request<?> request, String _prefix) {
        ...
        if (_availabilityZone.getSupportedPlatforms() != null) {
            prefix = _prefix + "SupportedPlatforms";
            java.util.List<SupportedPlatform> supportedPlatforms = _availabilityZone
                    .getSupportedPlatforms();
            int supportedPlatformsIndex = 1;
            String supportedPlatformsPrefix = prefix;
            for (SupportedPlatform supportedPlatformsItem : supportedPlatforms) {
                prefix = supportedPlatformsPrefix + "." + supportedPlatformsIndex;
                if (supportedPlatformsItem != null) {
                    SupportedPlatformStaxMarshaller.getInstance().marshall(supportedPlatformsItem,
                            request, prefix + ".");
                }
                supportedPlatformsIndex++;
            }
            prefix = supportedPlatformsPrefix;
        }
    }
